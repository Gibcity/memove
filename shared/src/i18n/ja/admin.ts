...
const admin: TranslationStrings = {
  ...
  'admin.weather.description':
    'memoveは天気データにOpen‑Meteoを使用しています。無料でオープンソース、APIキーは不要です。',
  ...
  'admin.update.dockerText':
    'memoveはDockerで実行されています。{version} に更新するには、サーバーで次のコマンドを実行してください：',
  'admin.update.nonDockerText':
    'このmemoveインスタンスはDockerで実行されていません。{version} に更新するには、使用したインストールまたは更新方法をもう一度実行してください。たとえばProxmox Community Scriptsの場合は、LXCコンソールから更新を実行します：',
  ...
  'admin.passkey.rpIdHint':
    'パスキーが紐づくドメイン名のみ（例：memove.example.org）。空欄の場合は APP_URL から導出されます。後で変更すると既存のパスキーは無効になります。',
  'admin.passkey.origins': '許可するオリジン',
  'admin.passkey.originsHint':
    'カンマ区切りの完全なオリジン（例：https://memove.example.org）。空欄の場合は APP_URL を使用します。',
