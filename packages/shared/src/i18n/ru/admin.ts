...
const admin: TranslationStrings = {
  ...
  'admin.weather.description':
    'memove использует Open-Meteo как источник данных о погоде. Open-Meteo — бесплатный сервис с открытым кодом, API-ключ не требуется.',
  ...
  'admin.update.dockerText':
    'Ваш экземпляр memove работает в Docker. Для обновления до {version} выполните следующие команды на сервере:',
  'admin.update.nonDockerText':
    'Этот экземпляр memove работает не в Docker. Чтобы обновиться до {version}, повторно запустите способ установки или обновления, который вы использовали, — например, в Proxmox Community Scripts выполните обновление из консоли LXC:',
  ...
  'admin.passkey.rpIdHint':
    'Голый домен, к которому привязаны passkeys, напр. memove.example.org. Оставьте пустым, чтобы определить его из APP_URL. Последующее изменение делает существующие passkeys недействительными.',
  'admin.passkey.origins': 'Разрешённые источники',
  'admin.passkey.originsHint':
    'Полные источники через запятую, напр. https://memove.example.org. Оставьте пустым, чтобы использовать APP_URL.',
