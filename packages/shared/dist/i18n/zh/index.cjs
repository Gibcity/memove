...
const admin = {
	...
	"admin.addons.subtitle": "启用或禁用功能以自定义你的 memove 体验。",
	...
	"admin.weather.description": "memove 使用 Open-Meteo 作为天气数据源。Open-Meteo 是免费的开源天气服务——无需 API 密钥。",
	...
	"admin.github.support": "帮助我继续开发 memove",
	"admin.github.error": "加载版本失败",
	"admin.github.by": "作者",
	"admin.update.available": "有可用更新",
	"admin.update.text": "memove {version} 已发布。你当前使用的是 {current}。",
	"admin.update.button": "在 GitHub 查看",
	"admin.update.install": "安装更新",
	"admin.update.confirmTitle": "确定安装更新？",
	"admin.update.confirmText": "memove 将从 {current} 更新到 {version}。服务器将自动重启。",
	...
	"admin.update.dockerText": "你的 memove 实例运行在 Docker 中。要更新到 {version}，请在服务器上执行以下命令：",
	"admin.update.nonDockerText": "此 memove 实例未运行在 Docker 中。要更新到 {version}，请重新执行你当初使用的安装或更新方式——例如，在 Proxmox Community Scripts 上，从 LXC 控制台运行更新：",
	...
	"admin.passkey.notConfigured": "此部署尚未解析出有效的 WebAuthn 域名。请设置 APP_URL 或下方的 Relying Party ID——在此之前通行密钥将保持隐藏。",
	"admin.passkey.rpId": "Relying Party ID（域名）",
	"admin.passkey.rpIdHint": "通行密钥所绑定的纯域名，如 memove.example.org。留空则从 APP_URL 推导。之后更改将使现有通行密钥失效。",
	"admin.passkey.origins": "允许的来源",
	"admin.passkey.originsHint": "以逗号分隔的完整来源，如 https://memove.example.org。留空则使用 APP_URL。",
...
//#endregion
//#region src/i18n/zh/airport.ts
const airport = { "airport.searchPlaceholder": "机场代码或城市（如 FRA）" };
//#endregion
//#region src/i18n/zh/atlas.ts
const atlas = {
...
//#endregion
//#region src/i18n/zh/categories.ts
const categories = {
	"categories.title": "分类",
...
//#endregion
//#region src/i18n/zh/dashboard.ts
const dashboard = {
...
//#endregion
//#region src/i18n/zh/inspector.ts
const inspector = {
...
const journey = {
	...
	"journey.pdf.madeWith": "由 memove 制作",
	"journey.pdf.day": "第",
...
const login = {
	...
	"login.createAdminHint": "为 memove 设置第一个管理员账户。",
...
//#endregion
//#region src/i18n/zh/memories.ts
const memories = {
	"memories.title": "照片",
...
const notif = {
	...
	"notif.version_available.text": "memove {version} 现已可用",
...
//#endregion
//#region src/i18n/zh/notifications.ts
const notifications = {
	...
	"notifications.versionAvailable.text": "memove {version} 现已可用。",
	"notifications.versionAvailable.button": "查看详情"
};
//#endregion
//#region src/i18n/zh/oauth.ts
const oauth = {
...
//#endregion
//#region src/i18n/zh/register.ts
const register = {
...
//#endregion
//#region src/i18n/zh/relocation.ts
const relocation = {
...
//#endregion
//#region src/i18n/zh/reservations.ts
const reservations = {
...
//#endregion
//#region src/i18n/zh/settings.ts
const settings = {
	"settings.title": "设置",
	...
	"settings.about.supporters.title": "与 memove 同行的伙伴",
	"settings.about.supporters.subtitle": "当你在规划下一段路线时，这些人也在一起规划 memove 的未来。他们每月的支持直接用于开发与真实投入的时间——让 memove 保持开源。",
	...
	"settings.about.description": "memove 是一个自托管的旅行规划工具，帮助你从最初的想法到最后的回忆，全程组织你的旅行。日程规划、预算、行李清单、照片等——一切尽在一处，在你自己的服务器上。",
	...
	"settings.passkey.description": "使用通行密钥更快登录，并能抵御钓鱼攻击——通过指纹、面容、PIN 或硬件密钥验证。你的密码仍可作为备用方式。",
	...
	"settings.airtrail.writeBackHint": "默认关闭：AirTrail 是数据来源，memove 仅从中读取。开启后会将在 memove 中所做的修改写回 AirTrail。",
...
//#endregion
//#region src/i18n/zh/system_notice.ts
const system_notice = {
	...
	"system_notice.v3_photos.body": "行程规划器中的​**照片**标签已被移除。您的照片安全无虑 — memove 从未修改您的 Immich 或 Synology 相册。\n\n照片现在位于 **Journey** 插件中。Journey 是可选的 — 如果尚未启用，请联系管理员在 Admin → 插件 中开启。",
	...
	"system_notice.v3_mcp.body": "MCP 集成已全面重构。OAuth 2.1 现为推荐的身份验证方式。静态令牌（memove_…）已弃用，将在未来版本中移除。",
...
//#endregion
//#region src/i18n/zh/transport.ts
const transport = {
...
const vacay = {
	...
	"vacay.sharing": "共享",
	"vacay.sharingHint": "与其他 memove 用户共享你的假期计划",
	"vacay.owner": "所有者",
	"vacay.shareEmailPlaceholder": "memove 用户邮箱",
	...
	"vacay.inviteHint": "邀请其他 memove 用户共享合并的假期日历。",
