...
const admin = {
	...
	"admin.addons.subtitle": "Включайте или отключайте функции для настройки memove под себя.",
	...
	"admin.weather.description": "memove использует Open-Meteo как источник данных о погоде. Open-Meteo — бесплатный сервис с открытым кодом, API-ключ не требуется.",
	...
	"admin.github.support": "Помогает продолжать разработку memove",
	"admin.github.error": "Не удалось загрузить релизы",
	"admin.github.by": "от",
	"admin.update.available": "Доступно обновление",
	"admin.update.text": "Доступна версия memove {version}. У вас установлена {current}.",
	...
	"admin.update.confirmText": "memove будет обновлён с {current} до {version}. Сервер перезапустится автоматически.",
	...
	"admin.update.dockerText": "Ваш экземпляр memove работает в Docker. Для обновления до {version} выполните следующие команды на сервере:",
	"admin.update.nonDockerText": "Этот экземпляр memove работает не в Docker. Чтобы обновиться до {version}, повторно запустите способ установки или обновления, который вы использовали, — например, в Proxmox Community Scripts выполните обновление из консоли LXC:",
	...
	"admin.passkey.loginHint": "Показывать вариант «Войти с помощью passkey» и разрешить пользователям регистрировать passkeys в своих настройках.",
	"admin.passkey.notConfigured": "Для этого развёртывания пока не определён домен WebAuthn. Задайте APP_URL или Relying Party ID ниже — до этого passkeys остаются скрытыми.",
	"admin.passkey.rpId": "Relying Party ID (домен)",
	"admin.passkey.rpIdHint": "Голый домен, к которому привязаны passkeys, напр. memove.example.org. Оставьте пустым, чтобы определить его из APP_URL. Последующее изменение делает существующие passkeys недействительными.",
	"admin.passkey.origins": "Разрешённые источники",
	"admin.passkey.originsHint": "Полные источники через запятую, напр. https://memove.example.org. Оставьте пустым, чтобы использовать APP_URL.",
	"admin.passkey.reset": "Сбросить passkeys",
	"admin.passkey.resetHint": "Удалить все passkeys этого пользователя (напр. при потере устройства). Он по-прежнему сможет войти по паролю.",
...
//#endregion
//#region src/i18n/ru/airport.ts
const airport = { "airport.searchPlaceholder": "Код аэропорта или город (напр. FRA)" };
//#endregion
//#region src/i18n/ru/atlas.ts
const atlas = {
...
//#endregion
//#region src/i18n/ru/categories.ts
const categories = {
...
//#endregion
//#region src/i18n/ru/dashboard.ts
const dashboard = {
...
//#endregion
//#region src/i18n/ru/inspector.ts
const inspector = {
...
const journey = {
	...
	"journey.pdf.madeWith": "Сделано с помощью memove",
...
const login = {
	...
	"login.createAdminHint": "Настройте первый аккаунт администратора для memove.",
...
//#endregion
//#region src/i18n/ru/memories.ts
const memories = {
	"memories.title": "Фото",
...
const notif = {
	...
	"notif.version_available.text": "memove {version} теперь доступен",
...
//#endregion
//#region src/i18n/ru/notifications.ts
const notifications = {
	...
	"notifications.versionAvailable.text": "memove {version} теперь доступен.",
	"notifications.versionAvailable.button": "Подробнее"
};
//#endregion
//#region src/i18n/ru/oauth.ts
const oauth = {
...
const places = {
	...
	"places.enrichOnImportHint": "Находит каждое импортированное место и добавляет фото, адрес и контакты. Требуется ключ Google Maps."
};
//#endregion
//#region src/i18n/ru/planner.ts
const planner = {
...
//#endregion
//#region src/i18n/ru/register.ts
const register = {
...
//#endregion
//#region src/i18n/ru/relocation.ts
const relocation = {
...
//#endregion
//#region src/i18n/ru/reservations.ts
const reservations = {
...
//#endregion
//#region src/i18n/ru/settings.ts
const settings = {
	...
	"settings.optimizeFromAccommodationHint": "При оптимизации дня маршрут начинается от отеля, в котором вы просыпаетесь, и заканчивается у того, в который вы заселяетесь вечером.",
	...
	"settings.about.supporters.subtitle": "Пока ты планируешь следующий маршрут, эти люди планируют вместе со мной будущее memove. Их ежемесячный взнос идёт напрямую в разработку и реально потраченные часы — чтобы memove оставался Open Source.",
	...
	"settings.about.description": "memove — это самостоятельно размещаемый планировщик путешествий, который помогает организовать поездки от первой идеи до последнего воспоминания. Планирование по дням, бюджет, списки вещей, фото и многое другое — всё в одном месте, на вашем собственном сервере.",
	...
	"settings.passkey.description": "Входите быстрее и с защитой от фишинга с помощью passkey — отпечатка пальца, лица, PIN-кода или аппаратного ключа. Ваш пароль остаётся как резервный способ.",
	"settings.passkey.notConfigured": "Passkeys включены, но ещё не полностью настроены на этом сервере. Попросите администратора задать домен WebAuthn.",
	...
	"settings.airtrail.writeBackHint": "По умолчанию выключено: AirTrail является источником истины, а memove только читает из него. Включите, чтобы отправлять изменения, сделанные в memove, обратно в AirTrail.",
...
//#endregion
//#region src/i18n/ru/system_notice.ts
const system_notice = {
	...
	"system_notice.v3_photos.body": "Вкладка **Фото** в Планировщике путешествий удалена. Ваши фото в безопасности — memove никогда не изменял вашу библиотеку Immich или Synology.\n\nФото теперь доступны в дополнении **Journey**. Journey необязателен — если он ещё недоступен, попросите администратора включить его в разделе Admin → Дополнения.",
	...
	"system_notice.v3_mcp.body": "Интеграция MCP была полностью переработана. OAuth 2.1 теперь является рекомендуемым методом аутентификации. Статические токены (memove_…) устарели и будут удалены в будущей версии.",
...
//#endregion
//#region src/i18n/ru/transport.ts
const transport = {
...
const vacay = {
	...
	"vacay.sharingHint": "Поделитесь планом отпуска с другими пользователями memove",
	"vacay.owner": "Владелец",
	"vacay.shareEmailPlaceholder": "Эл. почта пользователя memove",
	...
	"vacay.inviteHint": "Пригласите другого пользователя memove для совместного календаря отпусков.",
