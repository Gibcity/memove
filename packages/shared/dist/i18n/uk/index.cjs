...
const admin = {
	...
	"admin.addons.subtitle": "Увімкніть або вимкніть функції, щоб налаштувати memove під себе.",
	...
	"admin.weather.description": "memove використовує Open-Meteo як джерело даних про погоду. Open-Meteo — безкоштовний сервіс з відкритим кодом, API-ключ не потрібен.",
	...
	"admin.github.support": "Допомагає продовжувати розробку memove",
	"admin.github.error": "Не вдалося завантажити релізи",
	"admin.github.by": "від",
	"admin.update.available": "Доступне оновлення",
	"admin.update.text": "Доступна версія memove {version}. У вас встановлено {current}.",
	...
	"admin.update.confirmText": "memove буде оновлено з {current} до {version}. Сервер перезапуститься автоматично.",
	...
	"admin.update.dockerText": "Ваш екземпляр memove працює в Docker. Для оновлення до {version} виконайте ці команди на сервері:",
	"admin.update.nonDockerText": "Цей екземпляр memove не працює в Docker. Щоб оновити до {version}, повторно запустіть метод встановлення або оновлення, який ви використовували, — наприклад, у Proxmox Community Scripts запустіть оновлення з консолі LXC:",
	...
	"admin.passkey.cardHint": "Дозволити користувачам входити за допомогою passkey (WebAuthn). За замовчуванням вимкнено.",
	"admin.passkey.login": "Увімкнути вхід за допомогою passkey",
	"admin.passkey.loginHint": "Показувати опцію «Увійти за допомогою passkey» та дозволити користувачам реєструвати passkeys у своїх налаштуваннях.",
	"admin.passkey.notConfigured": "Для цього розгортання поки не визначено жодного домену WebAuthn. Вкажіть APP_URL або Relying Party ID нижче — до того часу passkeys залишатимуться прихованими.",
	"admin.passkey.rpId": "Relying Party ID (домен)",
	"admin.passkey.rpIdHint": "Чистий домен, до якого прив’язані passkeys, напр. memove.example.org. Залиште порожнім, щоб визначити його з APP_URL. Подальша зміна робить наявні passkeys недійсними.",
	"admin.passkey.origins": "Дозволені джерела (origins)",
	"admin.passkey.originsHint": "Повні джерела через кому, напр. https://memove.example.org. Залиште порожнім, щоб використати APP_URL.",
	"admin.passkey.reset": "Скинути passkeys",
	"admin.passkey.resetHint": "Видалити всі passkeys цього користувача (напр. у разі втрати пристрою). Він зможе входити за допомогою свого пароля.",
...
//#endregion
//#region src/i18n/uk/airport.ts
const airport = { "airport.searchPlaceholder": "Код аеропорту або місто (наприклад, FRA)" };
//#endregion
//#region src/i18n/uk/atlas.ts
const atlas = {
...
//#endregion
//#region src/i18n/uk/categories.ts
const categories = {
...
//#endregion
//#region src/i18n/uk/dashboard.ts
const dashboard = {
	...
	"dashboard.loadErrorBanner": "Не вдалося з'єднатися із сервером. Ваші поїздки в безпеці — будь ласка, спробуйте ще раз.",
...
//#endregion
//#region src/i18n/uk/inspector.ts
const inspector = {
...
const journey = {
	...
	"journey.pdf.madeWith": "Зроблено з допомогою memove",
...
const login = {
	...
	"login.createAdminHint": "Налаштуйте перший акаунт адміністратора для memove.",
...
//#endregion
//#region src/i18n/uk/memories.ts
const memories = {
	"memories.title": "Фото",
...
const notif = {
	...
	"notif.version_available.text": "memove {version} тепер доступний",
...
//#endregion
//#region src/i18n/uk/notifications.ts
const notifications = {
	...
	"notifications.versionAvailable.text": "memove {version} тепер доступний.",
	"notifications.versionAvailable.button": "Докладніше"
};
//#endregion
//#region src/i18n/uk/oauth.ts
const oauth = {
...
const places = {
	...
	"places.enrichOnImportHint": "Знаходить кожне імпортоване місце й додає фото, адресу та контакти. Потрібен ключ Google Maps."
};
//#endregion
//#region src/i18n/uk/planner.ts
const planner = {
...
//#endregion
//#region src/i18n/uk/register.ts
const register = {
...
//#endregion
//#region src/i18n/uk/relocation.ts
const relocation = {
...
//#endregion
//#region src/i18n/uk/reservations.ts
const reservations = {
...
//#endregion
//#region src/i18n/uk/settings.ts
const settings = {
	...
	"settings.optimizeFromAccommodationHint": "Під час оптимізації дня починайте маршрут від готелю, у якому ви прокидаєтеся, і завершуйте його тим, у який ви заселяєтеся ввечері.",
	...
	"settings.about.supporters.title": "Спутники memove",
	"settings.about.supporters.subtitle": "Поки ти плануєш наступний маршрут, ці люди разом зі мною планують майбутнє memove. Їхній щомісячний внесок йде безпосередньо в розробку та реальні витрачені години — щоб memove залишався Open Source.",
	...
	"settings.about.description": "memove — це self-hosted планувальник подорожей, який допомагає організувати поїздки від першої ідеї до останнього спогаду. Планування по днях, бюджет, списки речей, фото та багато іншого — все в одному місці, на вашому власному сервері.",
	...
	"settings.passkey.description": "Входьте швидше та з захистом від фішингу за допомогою passkey — відбитка пальця, обличчя, PIN-коду або апаратного ключа. Ваш пароль залишається як резервний варіант.",
	"settings.passkey.notConfigured": "Passkeys увімкнено, але цей сервер ще не повністю налаштовано. Попросіть адміністратора вказати домен WebAuthn.",
	...
	"settings.airtrail.writeBackHint": "Типово вимкнено: AirTrail є джерелом істини, а memove лише читає з нього. Увімкніть, щоб надсилати зміни, зроблені в memove, назад до AirTrail.",
...
//#endregion
//#region src/i18n/uk/system_notice.ts
const system_notice = {
	...
	"system_notice.v3_photos.body": "Вкладку **Фото** в Планувальнику подорожей видалено. Ваші фото в безпеці — memove ніколи не змінював вашу бібліотеку Immich або Synology.\n\nФото тепер доступні у доповненні **Journey**. Journey необов’язковий — якщо він ще недоступний, попросіть адміністратора включити його в розділі Адмін → Додатки.",
	...
	"system_notice.v3_mcp.body": "Інтеграція MCP була повністю перероблена. OAuth 2.1 тепер є рекомендованим методом автентифікації. Статичні токени (memove_…) застаріли і будуть видалені в майбутній версії.",
...
//#endregion
//#region src/i18n/uk/transport.ts
const transport = {
...
const vacay = {
	...
	"vacay.sharingHint": "Поділіться планом відпустки з іншими користувачами memove",
	"vacay.owner": "Власник",
	"vacay.shareEmailPlaceholder": "Ел. пошта користувача memove",
	...
	"vacay.inviteHint": "Запросіть іншого користувача memove для спільного календаря відпусток.",
