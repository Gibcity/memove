...
const admin = {
	...
	"admin.addons.subtitle": "기능을 활성화 또는 비활성화하여 memove 경험을 맞춤 설정하세요.",
	...
	"admin.weather.description": "memove은 날씨 데이터 소스로 Open-Meteo를 사용합니다. Open-Meteo는 무료 오픈 소스 날씨 서비스로 API 키가 필요 없습니다.",
	...
	"admin.github.by": "작성자",
	"admin.github.support": "memove 개발 지속에 도움이 됩니다",
	"admin.update.available": "업데이트 사용 가능",
	"admin.update.text": "memove {version}이(가) 사용 가능합니다. 현재 {current}을(를) 실행 중입니다.",
	"admin.update.button": "GitHub에서 보기",
	"admin.update.install": "업데이트 설치",
	"admin.update.confirmTitle": "업데이트를 설치할까요?",
	"admin.update.confirmText": "memove이 {current}에서 {version}으로 업데이트됩니다. 서버가 이후 자동으로 재시작됩니다.",
	...
	"admin.update.dockerText": "memove 인스턴스가 Docker에서 실행 중입니다. {version}으로 업데이트하려면 서버에서 다음 명령을 실행하세요:",
	"admin.update.nonDockerText": "이 memove 인스턴스는 Docker에서 실행되고 있지 않습니다. {version}으로 업데이트하려면 사용했던 설치 또는 업데이트 방법을 다시 실행하세요 — 예를 들어 Proxmox Community Scripts에서는 LXC 콘솔에서 업데이트를 실행하세요:",
	...
	"admin.passkey.notConfigured": "이 배포에 아직 WebAuthn 도메인이 확인되지 않습니다. 아래에서 APP_URL 또는 Relying Party ID를 설정하세요 — 그 전까지 패스키는 숨겨진 상태로 유지됩니다.",
	"admin.passkey.rpId": "Relying Party ID (도메인)",
	"admin.passkey.rpIdHint": "패스키가 바인딩되는 순수 도메인입니다. 예: memove.example.org. 비워두면 APP_URL에서 자동으로 가져옵니다. 나중에 변경하면 기존 패스키가 무효화됩니다.",
	"admin.passkey.origins": "허용된 오리진",
	"admin.passkey.originsHint": "쉼표로 구분된 전체 오리진입니다. 예: https://memove.example.org. 비워두면 APP_URL을 사용합니다.",
	"admin.passkey.reset": "패스키 초기화",
	"admin.passkey.resetHint": "이 사용자의 모든 패스키를 삭제합니다 (예: 기기 분실 시). 사용자는 비밀번호로 계속 로그인할 수 있습니다.",
...
//#endregion
//#region src/i18n/ko/airport.ts
const airport = { "airport.searchPlaceholder": "공항 코드 또는 도시 (예: ICN)" };
//#endregion
//#region src/i18n/ko/atlas.ts
const atlas = {
...
//#endregion
//#region src/i18n/ko/categories.ts
const categories = {
...
//#endregion
//#region src/i18n/ko/dashboard.ts
const dashboard = {
...
//#endregion
//#region src/i18n/ko/inspector.ts
const inspector = {
...
const journey = {
	...
	"journey.pdf.madeWith": "memove으로 만들어짐",
	"journey.pdf.day": "일차",
...
const login = {
	...
	"login.createAdminHint": "memove의 첫 번째 관리자 계정을 설정하세요.",
...
//#endregion
//#region src/i18n/ko/memories.ts
const memories = {
	"memories.title": "사진",
...
const notif = {
	...
	"notif.version_available.text": "memove {version}이(가) 사용 가능합니다",
...
//#endregion
//#region src/i18n/ko/notifications.ts
const notifications = {
	...
	"notifications.versionAvailable.text": "memove {version}이(가) 사용 가능합니다.",
...
//#endregion
//#region src/i18n/ko/register.ts
const register = {
...
//#endregion
//#region src/i18n/ko/relocation.ts
const relocation = {
...
//#endregion
//#region src/i18n/ko/reservations.ts
const reservations = {
...
//#endregion
//#region src/i18n/ko/settings.ts
const settings = {
	"settings.title": "설정",
	...
	"settings.optimizeFromAccommodationHint": "하루 일정을 최적화할 때, 아침에 머무는 숙소에서 경로를 시작하고 그날 저녁에 체크인하는 숙소에서 경로를 끝냅니다.",
	...
	"settings.about.supporters.title": "memove의 여행 동반자",
	"settings.about.supporters.subtitle": "다음 여정을 계획하는 동안, 이분들이 memove의 미래를 함께 만들어가고 있습니다. 월간 후원금은 개발과 실제 작업 시간에 직접 사용되어 memove이 오픈 소스로 유지될 수 있게 합니다.",
	...
	"settings.about.description": "memove은 첫 아이디어부터 마지막 추억까지 여행을 체계적으로 관리하는 자체 호스팅 여행 플래너입니다. 일별 계획, 예산, 짐 목록, 사진 등 모든 것이 하나의 서버에 담겨 있습니다.",
	...
	"settings.passkey.description": "지문, 얼굴, PIN 또는 하드웨어 키 같은 패스키로 더 빠르고 피싱에 강하게 로그인하세요. 비밀번호는 백업으로 그대로 유지됩니다.",
	"settings.passkey.notConfigured": "패스키가 활성화되어 있지만 이 서버에 아직 완전히 설정되지 않았습니다. 관리자에게 WebAuthn 도메인 설정을 요청하세요.",
	...
	"settings.airtrail.writeBackHint": "기본적으로 꺼져 있음: AirTrail이 신뢰할 수 있는 원본이며 memove은 읽기만 합니다. memove에서 변경한 내용을 AirTrail로 다시 보내려면 켜세요.",
...
//#endregion
//#region src/i18n/ko/system_notice.ts
const system_notice = {
	"system_notice.v3_photos.title": "3.0에서 사진이 이동했습니다",
	"system_notice.v3_photos.body": "여행 플래너의 **사진** 기능이 제거되었습니다. 사진은 안전합니다 — memove은 Immich 또는 Synology 라이브러리를 수정하지 않았습니다.\n\n사진은 이제 **Journey** 애드온에 있습니다. Journey는 선택 사항입니다 — 아직 사용할 수 없다면 관리자에게 관리자 → 애드온에서 활성화를 요청하세요.",
	...
	"system_notice.v3_mcp.body": "MCP 통합이 완전히 개선되었습니다. OAuth 2.1이 이제 권장 인증 방법입니다. 기존 정적 토큰 (memove_…)은 더 이상 사용되지 않으며 향후 릴리스에서 제거될 예정입니다.",
...
//#endregion
//#region src/i18n/ko/transport.ts
const transport = {
...
const vacay = {
	...
	"vacay.sharing": "공유",
	"vacay.sharingHint": "다른 memove 사용자와 휴가 계획을 공유합니다",
	"vacay.owner": "소유자",
	"vacay.shareEmailPlaceholder": "memove 사용자 이메일",
	...
	"vacay.inviteHint": "다른 memove 사용자를 초대하여 통합 휴가 캘린더를 공유하세요.",
