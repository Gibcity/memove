...
const admin = {
	...
	"admin.addons.subtitle": "機能を有効／無効にしてmemoveをカスタマイズします。",
	...
	"admin.weather.description": "memoveは天気データにOpen‑Meteoを使用しています。無料でオープンソース、APIキーは不要です。",
	...
	"admin.github.by": "作成者",
	"admin.github.support": "memoveの開発を支援",
	"admin.update.available": "更新があります",
	"admin.update.text": "memove {version} が利用可能です。現在は {current} を使用しています。",
	"admin.update.button": "GitHubで見る",
	"admin.update.install": "更新をインストール",
	"admin.update.confirmTitle": "更新をインストールしますか？",
	"admin.update.confirmText": "memoveを {current} から {version} に更新します。更新後、サーバーは自動的に再起動します。",
	...
	"admin.update.dockerText": "memoveはDockerで実行されています。{version} に更新するには、サーバーで次のコマンドを実行してください：",
	"admin.update.nonDockerText": "このmemoveインスタンスはDockerで実行されていません。{version} に更新するには、使用したインストールまたは更新方法をもう一度実行してください。たとえばProxmox Community Scriptsの場合は、LXCコンソールから更新を実行します：",
	...
	"admin.passkey.notConfigured": "このデプロイにはまだ有効な WebAuthn ドメインがありません。下の APP_URL または Relying Party ID を設定してください。それまでパスキーは表示されません。",
	"admin.passkey.rpId": "Relying Party ID（ドメイン）",
	"admin.passkey.rpIdHint": "パスキーが紐づくドメイン名のみ（例：memove.example.org）。空欄の場合は APP_URL から導出されます。後で変更すると既存のパスキーは無効になります。",
	"admin.passkey.origins": "許可するオリジン",
	"admin.passkey.originsHint": "カンマ区切りの完全なオリジン（例：https://memove.example.org）。空欄の場合は APP_URL を使用します。",
	"admin.passkey.reset": "パスキーをリセット",
	"admin.passkey.resetHint": "このユーザーのパスキーをすべて削除します（例：デバイスを紛失した場合）。パスワードでのサインインは引き続き可能です。",
...
//#endregion
//#region src/i18n/ja/airport.ts
const airport = { "airport.searchPlaceholder": "空港コードまたは都市名（例：FRA）" };
//#endregion
//#region src/i18n/ja/atlas.ts
const atlas = {
...
//#endregion
//#region src/i18n/ja/categories.ts
const categories = {
...
//#endregion
//#region src/i18n/ja/dashboard.ts
const dashboard = {
...
//#endregion
//#region src/i18n/ja/inspector.ts
const inspector = {
...
const login = {
	...
	"login.createAdminHint": "memoveの最初の管理者アカウントを設定します。",
...
//#endregion
//#region src/i18n/ja/memories.ts
const memories = {
	"memories.title": "写真",
...
const notif = {
	...
	"notif.version_available.text": "memove {version}が利用可能です",
...
//#endregion
//#region src/i18n/ja/notifications.ts
const notifications = {
	...
	"notifications.versionAvailable.text": "memove {version} が利用可能です。",
...
const places = {
	...
	"places.enrichOnImportHint": "インポートした各場所を検索して、写真・住所・連絡先を追加します。Google Maps キーが必要です。"
};
//#endregion
//#region src/i18n/ja/planner.ts
const planner = {
	"planner.places": "場所",
	"planner.bookings": "予約",
...
//#endregion
//#region src/i18n/ja/register.ts
const register = {
...
//#endregion
//#region src/i18n/ja/relocation.ts
const relocation = {
...
//#endregion
//#region src/i18n/ja/reservations.ts
const reservations = {
...
//#endregion
//#region src/i18n/ja/settings.ts
const settings = {
	"settings.title": "設定",
	...
	"settings.about.supporters.title": "memoveの旅仲間",
	"settings.about.supporters.subtitle": "皆さんの支援がmemoveの未来を支えています。",
	...
	"settings.about.description": "memoveはセルフホスト型の旅行プランナーです。",
	...
	"settings.passkey.description": "指紋、顔認証、PIN、またはハードウェアキーを使うパスキーで、より速く、フィッシングに強いサインインができます。パスワードはバックアップとして残ります。",
	"settings.passkey.notConfigured": "パスキーは有効ですが、このサーバーではまだ完全には設定されていません。管理者に WebAuthn ドメインの設定を依頼してください。",
	...
	"settings.airtrail.writeBackHint": "既定ではオフ: AirTrail が信頼できる情報源で、memove は読み取りのみを行います。memove で行った編集を AirTrail に書き戻すにはオンにします。",
...
//#endregion
//#region src/i18n/ja/system_notice.ts
const system_notice = {
	"system_notice.v3_photos.title": "写真の場所が3.0で変更されました",
	"system_notice.v3_photos.body": "旅行プランナー内の写真は削除されましたが、写真データは安全です。memoveがImmichやSynologyのライブラリを変更することはありません。\n\n写真は現在日記アドオンにあります。日記は任意機能です。未有効の場合は、管理画面 → アドオンで有効にしてください。",
	...
	"system_notice.v3_mcp.body": "MCP連携が全面的に刷新されました。OAuth 2.1が推奨認証方式です。従来の静的トークン（memove_…）は非推奨となり、将来削除されます。",
...
//#endregion
//#region src/i18n/ja/transport.ts
const transport = {
...
const vacay = {
	...
	"vacay.sharing": "共有",
	"vacay.sharingHint": "他のmemoveユーザーと休暇計画を共有",
	"vacay.owner": "所有者",
	"vacay.shareEmailPlaceholder": "memoveユーザーのメール",
	...
	"vacay.inviteHint": "別のmemoveユーザーを招待して、休暇カレンダーを共有します。",
