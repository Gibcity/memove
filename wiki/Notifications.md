...
memove supports four delivery channels. Which channels appear depends on what the admin has enabled server-side.
...
| **Webhook** | memove POSTs a JSON payload to a URL you specify. Discord and Slack webhook URLs are auto-detected and receive a natively formatted payload. |
...
|-------|-------------|---------|
| `version_available` | A new memove version is available | in-app, email, webhook, ntfy |
...
Enter a URL that memove will POST to when a notification fires. Once saved, the URL is displayed as `••••••••`. Use the **Test** button to send a test payload to the saved URL.

memove auto-detects the webhook destination and adjusts the payload format:
...
- **Generic** — sends a plain JSON object with `event`, `title`, `body`, `tripName`, `link`, `timestamp`, and `source` (`"memove"`) fields.
