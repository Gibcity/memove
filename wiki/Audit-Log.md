...
The audit log records significant actions taken on your memove instance. Use it to monitor logins, admin changes, and integration configuration.
...
The client IP is read from the `X-Forwarded-For` header. When memove is behind a reverse proxy, set `TRUST_PROXY=1` (the number of proxy hops) so the header is trusted and the real client IP is recorded. Without this setting, the proxy's own IP is logged instead. See [Environment-Variables](Environment-Variables).
