...
```bash
  -e TZ=Europe/Berlin \
  -e ALLOWED_ORIGINS=https://memove.example.com \
```
...
Replace `gibcity/memove:latest` in the run command with your chosen tag to pin to a major version or exact release.

## Volume Reference

| Volume | Container path | What lives there |
|---|---|---|
| `./data` | `/app/data` | `travel.db` (SQLite database), `logs/memove.log`, `.jwt_secret`, `.encryption_key` |
...
docker run -d \
  --name memove \
  -p 3000:3000 \
  -v memove_data:/app/data \
  -v memove_uploads:/app/uploads \
  -e ENCRYPTION_KEY=<your-32-byte-hex-key> \
  --restart unless-stopped \
  gibcity/memove:latest
```

Docker creates `memove_data` and `memove_uploads` automatically on first run. Named volumes are easier to manage with `docker volume` commands and work better in some NAS or container-management environments.
...
- [Reverse-Proxy](Reverse-Proxy) — HTTPS is required for PWA install and the `memove_session` cookie `secure` flag
