...
OAuth scopes control exactly which data your AI client can read or write in memove. You select scopes during the OAuth consent screen or when pre-creating an OAuth client. You can revoke access at any time by deleting the OAuth client or token from **Settings → Integrations → MCP**.

![OAuth consent screen](assets/OAuthConsentDCR.png)

## All scopes

memove defines 27 scopes across 13 groups.
