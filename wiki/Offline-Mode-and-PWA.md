...
memove can be installed as a Progressive Web App (PWA) and used without an internet connection for previously synced trips.

## Install as an app (PWA)

memove must be served over **HTTPS** — the install prompt does not appear on plain HTTP.

**iOS (Safari):**
1. Open memove in Safari.
2. Tap the Share button.
3. Select **Add to Home Screen**.

**Android (Chrome / Edge):**
1. Open memove in the browser.
2. Tap the browser menu.
3. Select **Install app** or **Add to Home Screen**.

Once installed, memove launches in **standalone** mode (fullscreen, no browser UI) using the memove icon.

## What works offline

memove uses Workbox service-worker caching plus an IndexedDB database (Dexie) for structured trip data. The following content is available offline after the first sync:
...
On login, after each trip-list refresh, and on WebSocket reconnect, memove runs a background sync that writes full trip bundles into IndexedDB:
