...
memove is a self-hosted, real-time collaborative travel planner licensed under AGPL-3.0.
...
- **MCP** — expose memove to AI assistants via the Model Context Protocol (OAuth 2.1)
...
> **Admin:** Most configuration lives in the Admin Panel. On first boot memove seeds an admin account automatically — credentials come from `ADMIN_EMAIL` / `ADMIN_PASSWORD` if set, otherwise a random password is printed to the container log.
