...
Putting memove behind a TLS-terminating reverse proxy is strongly recommended for production.
...
1. **WebSocket upgrades on `/ws`** — memove uses WebSockets for real-time sync. Set `proxy_read_timeout 86400` (Nginx) or rely on Caddy's automatic upgrade handling.
...
- `X-Forwarded-Proto $scheme` — tells memove whether the original request was HTTPS; required for `FORCE_HTTPS` redirect and cookie security to work correctly.
...
Four variables control how memove behaves behind a proxy. They work as a group:
...
If you access memove directly on `http://<host>:3000` without a proxy, leave `FORCE_HTTPS` unset and do not set `TRUST_PROXY`.
