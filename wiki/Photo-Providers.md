...
memove can browse your personal photo library on Immich or Synology Photos and attach selected photos to trips. memove never copies the original files — it stores only a reference (provider name + asset ID) and proxies all image streams through its own server, so your provider credentials are never sent to the browser.
...
| Mirror journey photos to Immich on upload | No | Checkbox; when enabled, photos you upload in memove are also pushed to your Immich library |

Enter the full URL of your Immich instance and an Immich API key. The API key is stored encrypted on the memove server and is never returned to the browser after it is saved.

#### Required API key permissions

When generating the API key in Immich (**Account Settings → API Keys**), grant only the scopes memove actually uses:

| Permission | Why memove needs it |
|------------|-------------------|
...
| `asset.upload` | *Only if you enable "Mirror journey photos to Immich on upload"* — push memove uploads back to your library |

memove never modifies or deletes anything in Immich, so no `update`, `delete`, or admin scopes are needed.
...
Synology Photos doesn't use API keys — memove signs in with a regular DSM user account. To minimize blast radius, create a **dedicated low-privilege DSM user** for memove rather than reusing your admin account:
...
- 2FA is supported — enter the OTP at first connection; memove stores the resulting device token so you won't be re-prompted on subsequent saves.
- Read-only access is enough — memove only lists albums, lists items, runs searches, and fetches thumbnails. It never writes, uploads, or deletes.
...
You can configure both Immich and Synology simultaneously. memove queries photos from all enabled providers when loading trip photos.
