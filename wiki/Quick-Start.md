...
Get memove running in under five minutes with a single Docker command.
...
**Why the encryption key?** memove encrypts stored secrets (API keys, MFA seeds, OIDC credentials) using this key. If you skip it, memove auto-generates one and saves it to `./data/.encryption_key`. Setting it explicitly means you control the key and can back it up separately.
...
- [Reverse-Proxy](Reverse-Proxy) — put memove behind HTTPS (required for PWA install and secure cookies)
