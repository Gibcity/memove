...
memove uses **Leaflet** by default. If you configure a Mapbox access token in Settings → Map, the map upgrades to **Mapbox GL** with higher-quality tiles, 3D buildings, and terrain. If Mapbox GL is selected but no access token is present, memove falls back to Leaflet automatically so the map is never blank.
...
> **Requires:** Settings → Display → **Route calculation** must be ON. When this setting is OFF, memove never queries the routing service, so no pills are calculated or drawn at any zoom level.
