...
OpenID Connect (OIDC) lets users log in with an existing identity provider — Google, Authentik, Keycloak, or any OIDC-compatible IdP — instead of a local email/password. On first SSO login, a memove account is created automatically using the email from the provider.
...
When an SSO login matches an existing memove account by email or OIDC subject (`sub`), that account is used and the OIDC identity is linked automatically. If no matching account exists, memove attempts to create one. The outcome depends on the following:
