...
Demo mode lets you run a public "try before you install" instance of memove. A shared demo account is available for visitors, write operations are blocked for that account, and the database resets automatically every hour so the instance stays in a known state.

<!-- TODO: screenshot: demo mode banner or try-demo button on login page -->

## Enabling demo mode

Set `DEMO_MODE=true` in your environment and restart memove. See [Environment-Variables](Environment-Variables) for how to set environment variables.
...
memove schedules an automatic hourly reset of the demo database. At each reset:
...
On first start with demo mode active, memove seeds three example trips (Tokyo & Kyoto, Barcelona Long Weekend, New York City) owned by the admin and shared with the demo user, then saves the initial baseline automatically.
