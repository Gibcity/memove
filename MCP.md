...
memove includes a built-in [Model Context Protocol](https://modelcontextprotocol.io/) (MCP) server that lets AI
assistants — such as Claude Desktop, Cursor, or any MCP-compatible client — read and modify your trip data through a
structured API.

> **Note:** MCP is an addon that must be enabled by your memove administrator before it becomes available.
...
4. Your browser opens memove's consent screen, where you choose which scopes (permissions) to grant.
...
> **Requirement:** The `APP_URL` environment variable must be set to your memove instance's public URL for OAuth
...
memove's MCP server supports three authentication methods. OAuth 2.1 is the recommended path for all external clients.
...
When connecting via OAuth 2.1, you grant specific scopes during the consent step. memove registers only the MCP tools
...
| **Demo mode restrictions**              | If memove is running in demo mode, all write operations through MCP are blocked.                                                                   |
...
Resources provide read-only access to your memove data. MCP clients can read these to understand the current state before
making changes.
...
memove exposes tools organized by feature area. Use `get_trip_summary` as a starting point — it returns everything about a
...
Before writing anything to memove, do some research: look up what's worth
...
Once you have a solid plan, write the whole thing to memove:
- Create the trip
