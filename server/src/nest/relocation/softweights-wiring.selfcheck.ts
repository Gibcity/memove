...
import type { UserProfile } from '@memove/shared';
...
function resolveWeights(
  filters: { weights?: Record<string, number>; softWeights?: Record<string, number> },
  userId: string | undefined,
  userProfiles: Map<string, UserProfile>,
): { weights: Record<string, number>; weightsFromProfile: boolean } {
...
// Case 1: caller-supplied weights win, profile ignored
{
  const r = resolveWeights(
    { weights: { cost: 5, climate: 5, safety: 5, healthcare: 5, jobs: 5, outdoors: 5 } },
    userId,
    profiles,
  );
...
// Case 2: no filters.weights, profile present → mapped
{
  ...
  check(
    'profile mapped to engine keys',
    r.weightsFromProfile === true &&
      r.weights.cost === 0.4 &&
      ...
      r.weights.outdoors === DEFAULT_WEIGHTS.outdoors,
...
// Case 4: userId but no profile → DEFAULT_WEIGHTS
{
  ...
  check(
    'default weights when userId has no profile',
    r.weightsFromProfile === false && r.weights === DEFAULT_WEIGHTS,
  );
}
...
// Case 5: profile with empty softWeights → DEFAULT_WEIGHTS
{
  profiles.set('u-empty', {
    ...profiles.get(userId)!,
    userId: 'u-empty',
    softWeights: {} as UserProfile['softWeights'],
  });
  const r = resolveWeights({}, 'u-empty', profiles);
  check(
    'default weights when profile has no softWeights',
    r.weightsFromProfile === false && r.weights === DEFAULT_WEIGHTS,
  );
}
