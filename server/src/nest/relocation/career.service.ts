...
// ponytail: hardcoded data — licensing boards are static URLs, not a DB table.
// NCSBN directory fallback for nursing where a state's exact BoN URL isn't verified:
// https://www.ncsbn.org/contact-bon.htm
type Profession = 'nursing' | 'teaching' | 'real_estate' | 'contractor';

function board(
  name: string,
  url: string,
): LicensingBoard { return { name, url }; }

const LICENSING_BOARDS: Record<string, Record<Profession, LicensingBoard>> = {
...
function licensingFallback(state: string): Record<Profession, LicensingBoard> {
  const stateName = stateNameFromCode(state);
  return {
    nursing: board(
      `${stateName} Board of Nursing — NCSBN directory`,
      'https://www.ncsbn.org/contact-bon.htm',
    ),
    teaching: board(
      `${stateName} Dept of Education (verify on .gov)`,
      `https://www.google.com/search?q=${encodeURIComponent(stateName + ' department of education official site .gov')}`,
    ),
    real_estate: board(
      `${stateName} Real Estate Commission (verify on .gov)`,
      `https://www.google.com/search?q=${encodeURIComponent(stateName + ' real estate commission licensing official .gov')}`,
    ),
    contractor: board(
      `${stateName} Contractor Licensing (verify on .gov)`,
      `https://www.google.com/search?q=${encodeURIComponent(stateName + ' contractor licensing board official .gov')}`,
    ),
  };
}
...
export interface OccupationOutlook {
  occupation: string;
  blsOohUrl: string;
  note: string;
}
...
export class CareerService {
  ...
  getLicensingBoards(state: string): Record<Profession, LicensingBoard> {
  ...
  getOccupationOutlook(occupation: string): OccupationOutlook {
