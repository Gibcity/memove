...
function generateAccessToken(): string {
  return 'memove_oa_' + randomBytes(32).toString('hex');
}

function generateRefreshToken(): string {
  return 'memove_rf_' + randomBytes(32).toString('hex');
}
...
export function createOAuthClient(
  ...
  const rawSecret   = isPublic ? null : 'memove_cs_' + randomBytes(24).toString('hex');
...
export function rotateOAuthClientSecret(
  ...
  const rawSecret  = 'memove_cs_' + randomBytes(24).toString('hex');
