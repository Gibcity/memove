...
describe('createOAuthClient', () => {
  it('creates a client successfully and returns client_secret only on creation', () => {
    ...
    expect((result.client!.client_secret as string).startsWith('memove_cs_')).toBe(true);
  });
...
describe('rotateOAuthClientSecret', () => {
  it('rotates secret and returns new client_secret starting with memove_cs_', () => {
    const { user } = createUser(testDb);
    const created = makeClient(user.id);
    ...
    expect((result.client_secret as string).startsWith('memove_cs_')).toBe(true);
...
describe('issueTokens + getUserByAccessToken', () => {
  it('issues tokens with correct prefixes', () => {
    ...
    expect(tokens.access_token.startsWith('memove_oa_')).toBe(true);
    expect(tokens.refresh_token.startsWith('memove_rf_')).toBe(true);
  ...
  it('getUserByAccessToken returns null for unknown token', () => {
    expect(getUserByAccessToken('memove_oa_unknown')).toBeNull();
  });
...
describe('refreshTokens', () => {
  it('exchanges a refresh token for a new token pair', () => {
    ...
    expect(result.tokens!.access_token.startsWith('memove_oa_')).toBe(true);
  });
  ...
  it('returns invalid_grant for unknown refresh token', () => {
    ...
    const result = refreshTokens('memove_rf_unknown', clientId, rawSecret);
