...
function setSmtp(): void {
  ...
  setAppSetting(testDb, 'smtp_from', 'memove@test.com');
}
...
function setUserNtfyTopic(userId: number, topic = 'my-memove-topic'): void {
  testDb.prepare("INSERT OR REPLACE INTO settings (user_id, key, value) VALUES (?, 'ntfy_topic', ?)").run(userId, topic);
}

function setAdminNtfyTopic(topic = 'memove-admin-alerts'): void {
