...
/**
 * An AirTrail flight as GET returns it, including the fields memove doesn't model.
...
function existingFlight(
  ...
  return {
    ...
    // AirTrail-owned detail memove never surfaces — must survive a writeback (#1240).
...
/** A linked memove reservation (the shape getReservationWithJoins returns). */
...
describe('airtrailSync.buildSavePayload', () => {
  it('round-trips the AirTrail-owned fields memove does not model (issue #1240)', () => {
  ...
  it('writes the memove time to the SCHEDULED fields so it round-trips on the next pull', () => {
    // Import reads the scheduled time, so a memove edit must be pushed back there
  ...
  it('blanks the scheduled time when the memove reservation has only a date', () => {
  ...
  it('preserves AirTrail aircraft/airline/reason when memove metadata omits them (#1240)', () => {
    // A memove edit can drop these AirTrail-owned fields from metadata; the writeback
