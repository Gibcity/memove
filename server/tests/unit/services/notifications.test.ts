...
describe('buildWebhookBody', () => {
  ...
  it('Discord embed footer defaults to memove when no trip name', () => {
    ...
    expect(body.embeds[0].footer.text).toBe('memove');
  });
  ...
  it('generic URL produces plain JSON with original fields plus timestamp and source', () => {
    ...
    expect(body).toHaveProperty('source', 'memove');
  });
});
...
describe('buildEmailHtml', () => {
  ...
  it('uses English i18n strings for lang=en', () => {
    ...
    expect(html).toContain('notifications enabled in memove');
  });

  it('uses German i18n strings for lang=de', () => {
    const html = buildEmailHtml('Subject', 'Body', 'de');
    expect(html).toContain('memove aktiviert');
  });
  ...
  it('falls back to English i18n for unknown language', () => {
    ...
    expect(unknown).toContain('notifications enabled in memove');
  });
});
