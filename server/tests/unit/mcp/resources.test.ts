...
describe('Resource: memove://trips', () => {
  it('returns all trips the user owns or is a member of', async () => {
    ...
    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: 'memove://trips' });
...
describe('Resource: memove://trips/{tripId}', () => {
  it('returns trip data for an accessible trip', async () => {
    const { user } = createUser(testDb);
    const trip = createTrip(testDb, user.id, { title: 'Paris Trip' });

    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: `memove://trips/${trip.id}` });
  ...
  it('returns access denied for inaccessible trip', async () => {
    ...
    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: `memove://trips/${otherTrip.id}` });
      const data = parseResourceResult(result) as any;
  ...
  it('returns access denied for non-existent ID', async () => {
    const { user } = createUser(testDb);
    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: 'memove://trips/99999' });
      const data = parseResourceResult(result) as any;
      expect(data.error).toBeTruthy();
    });
  });
...
describe('Resource: memove://trips/{tripId}/days', () => {
  it('returns days with assignments in order', async () => {
    ...
    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: `memove://trips/${trip.id}/days` });
...
describe('Resource: memove://trips/{tripId}/places', () => {
  it('returns all places for a trip', async () => {
    ...
    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: `memove://trips/${trip.id}/places` });
...
describe('Resource: memove://trips/{tripId}/budget', () => {
  it('returns budget items for a trip', async () => {
    ...
    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: `memove://trips/${trip.id}/budget` });
...
describe('Resource: memove://trips/{tripId}/packing', () => {
  it('returns packing items for a trip', async () => {
    ...
    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: `memove://trips/${trip.id}/packing` });
      const items = parseResourceResult(result) as any[];
      expect(items).toHaveLength(1);
...
describe('Resource: memove://trips/{tripId}/reservations', () => {
  it('returns reservations for a trip', async () => {
    ...
    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: `memove://trips/${trip.id}/reservations` });
      const items = parseResourceResult(result) as any[];
      expect(items).toHaveLength(1);
...
describe('Resource: memove://trips/{tripId}/days/{dayId}/notes', () => {
  it('returns notes for a specific day', async () => {
    ...
    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: `memove://trips/${trip.id}/days/${day.id}/notes` });
  ...
  it('returns access denied for invalid dayId', async () => {
    ...
    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: `memove://trips/${trip.id}/days/abc/notes` });
      const data = parseResourceResult(result) as any;
      expect(data.error).toBeTruthy();
    });
  });
...
describe('Resource: memove://trips/{tripId}/accommodations', () => {
  it('returns accommodations for a trip', async () => {
    ...
    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: `memove://trips/${trip.id}/accommodations` });
      const items = parseResourceResult(result) as any[];
      expect(items).toHaveLength(1);
...
describe('Resource: memove://trips/{tripId}/members', () => {
  it('returns owner and collaborators', async () => {
    ...
    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: `memove://trips/${trip.id}/members` });
      const data = parseResourceResult(result) as any;
...
describe('Resource: memove://trips/{tripId}/collab-notes', () => {
  it('returns collab notes with username', async () => {
    ...
    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: `memove://trips/${trip.id}/collab-notes` });
      const notes = parseResourceResult(result) as any[];
      expect(notes).toHaveLength(1);
...
describe('Resource: memove://categories', () => {
  it('returns all categories', async () => {
    const { user } = createUser(testDb);

    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: 'memove://categories' });
...
describe('Resource: memove://bucket-list', () => {
  it('returns only the current user\'s bucket list items', async () => {
    ...
    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: 'memove://bucket-list' });
      const items = parseResourceResult(result) as any[];
      expect(items).toHaveLength(1);
...
describe('Resource: memove://visited-countries', () => {
  it('returns only the current user\'s visited countries', async () => {
    ...
    await withHarness(user.id, async (harness) => {
      const result = await harness.client.readResource({ uri: 'memove://visited-countries' });
