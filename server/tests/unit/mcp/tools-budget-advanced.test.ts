 ...
 * Resources: memove://trips/{tripId}/budget/per-person, memove://trips/{tripId}/budget/settlement.
 */
...
describe('Resource: memove://trips/{tripId}/budget/per-person', () => {
  it('returns array for trip with no items', async () => {
    const { user } = createUser(testDb);
    const trip = createTrip(testDb, user.id);
    await withResourceHarness(user.id, async (h) => {
      const result = await h.client.readResource({ uri: `memove://trips/${trip.id}/budget/per-person` });
...
describe('Resource: memove://trips/{tripId}/budget/settlement', () => {
  it('returns settlement object for trip with no items', async () => {
    const { user } = createUser(testDb);
    const trip = createTrip(testDb, user.id);
    await withResourceHarness(user.id, async (h) => {
      const result = await h.client.readResource({ uri: `memove://trips/${trip.id}/budget/settlement` });
      const data = JSON.parse(result.contents[0].text as string);
