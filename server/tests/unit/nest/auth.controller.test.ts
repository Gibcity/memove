...
describe('AuthController (authenticated)', () => {
  ...
  it('avatar 403 in demo mode, 400 without a file, else saves', async () => {
    ...
    expect(await thrownAsync(() => new AuthController(asvc(), rl()).avatar(user, { filename: 'a.jpg' } as Express.Multer.File))).toEqual({ status: 403, body: { error: 'Uploads are disabled in demo mode. Self-host memove for full functionality.' } });
