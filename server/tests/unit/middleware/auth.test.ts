...
describe('extractToken', () => {
  it('returns cookie value when memove_session cookie is set', () => {
    const req = makeReq({ cookies: { memove_session: 'cookie-token' } });
  ...
  it('prefers cookie over Authorization header when both are present', () => {
    const req = makeReq({
      cookies: { memove_session: 'cookie-token' },
...
describe('authenticate', () => {
  ...
  it('returns 401 when JWT is invalid', () => {
    ...
    authenticate(makeReq({ cookies: { memove_session: 'invalid.jwt.token' } }), res, next);
    expect(next).not.toHaveBeenCalled();
    expect(status).toHaveBeenCalledWith(401);
  });
  ...
  it('AUTH-MW-003: calls next() and sets req.user for a valid JWT', () => {
    ...
    const req = makeReq({ cookies: { memove_session: token } });
    const next = vi.fn() as unknown as NextFunction;
    const { res } = makeRes();
  ...
  it('AUTH-MW-004: returns 401 for a valid JWT when user does not exist in DB', () => {
    ...
    authenticate(makeReq({ cookies: { memove_session: token } }), res, next);
  ...
  it('AUTH-MW-005: returns 401 for an expired JWT', () => {
    ...
    const next = vi.fn() as unknown as NextFunction;
    const { res, status } = makeRes();
    authenticate(makeReq({ cookies: { memove_session: expiredToken } }), res, next);
    expect(next).not.toHaveBeenCalled();
    expect(status).toHaveBeenCalledWith(401);
  });
  ...
  it('AUTH-MW-006: returns 401 for a JWT signed with the wrong secret', () => {
    ...
    const next = vi.fn() as unknown as NextFunction;
    const { res, status } = makeRes();
    authenticate(makeReq({ cookies: { memove_session: tamperedToken } }), res, next);
    expect(next).not.toHaveBeenCalled();
    expect(status).toHaveBeenCalledWith(401);
  });
