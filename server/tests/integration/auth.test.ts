...
describe('Login', () => {
  it('AUTH-001 — successful login returns 200, user object, and memove_session cookie', async () => {
    ...
    const cookies: string[] = Array.isArray(res.headers['set-cookie'])
      ...
      : [res.headers['set-cookie']];
    expect(cookies.some((c: string) => c.includes('memove_session'))).toBe(true);
  });
  ...
  it('AUTH-013 — POST /api/auth/logout clears session cookie', async () => {
    ...
    const sessionCookie = cookies.find((c: string) => c.includes('memove_session'));
...
describe('Demo login', () => {
  ...
  it('AUTH-022 — POST /api/auth/demo-login with DEMO_MODE and demo user returns 200 + cookie', async () => {
    testDb.prepare(
      "INSERT INTO users (username, email, password_hash, role) VALUES ('demo', 'demo@memove.app', 'x', 'user')"
    ).run();
    process.env.DEMO_MODE = 'true';
    try {
      const res = await request(app).post('/api/auth/demo-login');
      expect(res.status).toBe(200);
      expect(res.body.user.email).toBe('demo@memove.app');
    } finally {
      delete process.env.DEMO_MODE;
    }
  });
...
describe('Account deletion', () => {
  it('AUTH-040 — DELETE /auth/me succeeds when user has FK references', async () => {
    ...
    const trekPhotoRow = testDb.prepare(
      "INSERT INTO memove_photos (provider, asset_id, owner_id) VALUES ('immich', 'asset-auth-test', ?)"
    ).run(target.id);
