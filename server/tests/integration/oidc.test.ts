...
describe('GET /api/auth/oidc/callback', () => {
  it('OIDC-004: valid code for existing user → redirects to frontend with oidc_code', async () => {
    ...
    const res = await request(app).get(`/api/auth/oidc/callback?code=authcode123&state=${state}`).set('Cookie', `memove_oidc_state=${state}`);
  ...
  it('OIDC-005: new user gets created when registration is open', async () => {
    ...
    const res = await request(app).get(`/api/auth/oidc/callback?code=code999&state=${state}`).set('Cookie', `memove_oidc_state=${state}`);
  ...
  it('OIDC-009: token exchange failure → redirects with token_failed error', async () => {
    ...
    const res = await request(app).get(`/api/auth/oidc/callback?code=badcode&state=${state}`).set('Cookie', `memove_oidc_state=${state}`);
  ...
  it('OIDC-010a: missing id_token in token response → redirects with no_id_token error', async () => {
    ...
    const res = await request(app).get(`/api/auth/oidc/callback?code=anycode&state=${state}`).set('Cookie', `memove_oidc_state=${state}`);
...
describe('GET /api/auth/oidc/exchange', () => {
  it('OIDC-011: valid auth code returns JWT and sets cookie', async () => {
    ...
    expect(cookieHeader).toContain('memove_session');
  });
