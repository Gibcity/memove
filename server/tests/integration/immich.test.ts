...
describe('Immich album links', () => {
  ...
  it('IMMICH-022 — DELETE album-links removes associated photos but not individually-added ones', async () => {
    ...
    for (const assetId of ['asset-001', 'asset-002']) {
      testDb.prepare('INSERT OR IGNORE INTO memove_photos (provider, asset_id, owner_id) VALUES (?, ?, ?)').run('immich', assetId, user.id);
      const tkp = testDb.prepare('SELECT id FROM memove_photos WHERE provider = ? AND asset_id = ? AND owner_id = ?').get('immich', assetId, user.id) as any;
    ...
    testDb.prepare('INSERT OR IGNORE INTO memove_photos (provider, asset_id, owner_id) VALUES (?, ?, ?)').run('immich', 'asset-manual', user.id);
    const tkpManual = testDb.prepare('SELECT id FROM memove_photos WHERE provider = ? AND asset_id = ? AND owner_id = ?').get('immich', 'asset-manual', user.id) as any;
  ...
  it('IMMICH-023 — DELETE album-link by non-member returns 404', async () => {
    ...
    testDb.prepare('INSERT OR IGNORE INTO memove_photos (provider, asset_id, owner_id) VALUES (?, ?, ?)').run('immich', 'asset-owned', owner.id);
    const tkpOwned = testDb.prepare('SELECT id FROM memove_photos WHERE provider = ? AND asset_id = ? AND owner_id = ?').get('immich', 'asset-owned', owner.id) as any;
