...
import { z } from 'zod';
import type { RelocationService } from '../nest/relocation/relocation.service';
import type { RelocationJourneyService } from '../nest/relocation/relocation-journey.service';
import { TOOL_ANNOTATIONS_READONLY, TOOL_ANNOTATIONS_WRITE } from './tools/_shared';
...
// Handlers receive the whole services bag so tool bodies stay close to where
// they live. Cost / admin tools bind `relocation` for getLocationById and may
// ignore it; journey-only tools bind only the journey service. Per-tool files
// supply whichever services they have — `journey` may be undefined for core
// tools that don't touch journey state.
export interface RelocationServices {
  relocation: RelocationService;
  journey?: RelocationJourneyService;
}

// ponytail: third arg `userId` is the per-request user, present on tools that
// touch per-user state (profile, journey) and unused (with leading _) on pure
// data tools. The MCP adapter always supplies it; chat-service supplies it too.
export type ToolHandler<S extends RelocationServices = RelocationServices> = (
  svc: S,
...
export interface ToolDefinition {
  ...
  // ponytail: read vs write so the MCP adapter can filter with canRead/canWrite
  // exactly like the original code did. Replaces the per-file `if (R)` / `if (W)`
  // gating that was inline before.
  scope: 'read' | 'write';
}
...
const relocationTools: ToolDefinition[] = [
  {
    ...
    inputSchema: {
      states: z.array(z.string()).optional().describe("State codes to include (e.g., ['TX','FL','TN'])"),
      excludeStates: z.array(z.string()).optional().describe('State codes to exclude'),
      maxHomeValue: z.number().optional().describe('Maximum median home value in USD'),
      maxRent: z.number().optional().describe('Maximum median monthly rent in USD'),
      maxViolentCrime: z.number().optional().describe('Maximum violent crime rate per 100k'),
      maxRiskTornado: z.number().optional().describe('Maximum FEMA tornado risk score (0-100)'),
      maxRiskHurricane: z.number().optional().describe('Maximum FEMA hurricane risk score'),
      maxRiskEarthquake: z.number().optional().describe('Maximum FEMA earthquake risk score'),
      maxRiskWildfire: z.number().optional().describe('Maximum FEMA wildfire risk score'),
      maxHotDays: z.number().optional().describe('Max days >90°F per year'),
      maxColdDays: z.number().optional().describe('Max days <32°F per year'),
      nameContains: z.string().optional().describe('Filter by name (case-insensitive)'),
      limit: z.number().int().positive().optional().default(20).describe('Max results to return'),
    },
    annotations: TOOL_ANNOTATIONS_READONLY,
    scope: 'read',
    handler: (svc, args, _userId) => svc.relocation.searchLocations(args as never),
  },
  {
    ...
    annotations: TOOL_ANNOTATIONS_READONLY,
    scope: 'read',
    handler: (svc, args, _userId) => svc.relocation.scoreLocations(args as never),
  },
  {
    ...
    inputSchema: {
      locationIds: z.array(z.string()).min(2).describe("Location IDs to compare (e.g., ['austin-tx','nashville-tn'])"),
      weights,
    },
    annotations: TOOL_ANNOTATIONS_READONLY,
    scope: 'read',
    handler: (svc, args, _userId) =>
      svc.relocation.compareLocations(
        args.locationIds as string[],
  ...
  },
  {
    ...
    inputSchema: {
      locationId: z.string().describe("Location ID (e.g., 'memphis-tn') or partial name match"),
      weights,
    },
    annotations: TOOL_ANNOTATIONS_READONLY,
    scope: 'read',
    handler: (svc, args, _userId) =>
      svc.relocation.explainScore(args.locationId as string, args.weights as Record<string, number> | undefined),
  },
  {
    name: 'fiscal_health',
    description:
    ...
    inputSchema: {
      locationId: z.string().describe("Location ID (e.g., 'chicago-il') or partial name match"),
    },
    annotations: TOOL_ANNOTATIONS_READONLY,
    scope: 'read',
    handler: (svc, args, _userId) => svc.relocation.fiscalHealth(args.locationId as string),
  },
  {
    ...
    inputSchema: {
      softWeights: z.record(z.string(), z.number()).optional().describe('Metric → weight map (e.g., {"cost": 0.35, "climate": 0.25})'),
      hardFilters: z
        .array(
          z.object({
            field: z.string(),
        ...
        .optional(),
      statedPriorities: z
        .array(z.object({ metric: z.string(), rank: z.number(), weight: z.number().optional() }))
        .optional(),
    },
    ...
    handler: (svc, args, userId) =>
      svc.relocation.upsertUserProfile(userId, args),
  },
  {
...
const costTools: ToolDefinition[] = [
  {
    ...
    inputSchema: {
      locationIds: z.array(z.string()).min(2).describe("Location IDs to compare (e.g., ['austin-tx','nashville-tn'])"),
      householdIncome: z.number().positive().optional().describe('Optional household income — enables income-tax line items in the breakdown'),
      householdSize: z.number().int().positive().optional().default(2).describe('Household size for food/healthcare scaling (default 2)'),
    },
    annotations: TOOL_ANNOTATIONS_READONLY,
    scope: 'read',
    handler: (_svc, args, _userId) => {
      ...
      const ids = args.locationIds as string[];
      ...
      for (const [id, data] of Object.entries(breakdown)) {
        ...
        const d: any = data;
        const base: any = breakdown[baseKey!];
        deltas[id] = {
      ...
      return {
        baseline: baseKey,
        householdSize: (args.householdSize as number | undefined) ?? 2,
  ...
  },
  {
    ...
    description:
      "Calculate the net tax impact of moving between two locations. Returns state income tax delta (simplified 12% federal flat + state rate), property tax delta on the median home, and combined annual tax burden.",
    inputSchema: {
      ...
      homeValue: z.number().positive().optional().describe('Home value to use for property-tax comparison (defaults to destination median if omitted)'),
    },
    annotations: TOOL_ANNOTATIONS_READONLY,
    scope: 'read',
    handler: (_svc, args, _userId) => {
      const from = resolveLocation(args.fromLocationId as string);
      const to = resolveLocation(args.toLocationId as string);
      if (!from || !to) {
  ...
  },
  {
    ...
    description:
      'What salary does the user need in city B to maintain the same standard of living they have in city A?',
    inputSchema: {
    ...
    handler: (_svc, args, _userId) => {
      ...
      const currentSalary = args.currentSalary as number;
      ...
      const householdSize = (args.householdSize as number | undefined) ?? 2;
  ...
  },
  {
    name: 'cost_breakdown',
    description:
      'Detailed cost-of-living breakdown for a single US metro.',
    inputSchema: {
    ...
    scope: 'read',
    handler: (_svc, args, _userId) => {
      const loc = resolveLocation(args.locationId as string);
      if (!loc) return { __error: `Unknown location: ${args.locationId}` };
      const householdSize = (args.householdSize as number | undefined) ?? 2;
      const b = buildBreakdown(loc, householdSize, args.homeValue as number | undefined);
      ...
      return {
        ...
        notes: {
          ...
          homeValueUsed: (args.homeValue as number | undefined) ?? b.housing.medianHomeValue,
        },
        methodology:
...
const communityTools: ToolDefinition[] = [
  {
    name: 'assess_healthcare_access',
    description:
      'Detailed healthcare access analysis for a location — hospital density, physician supply, life expectancy vs national average, and specialist-specific guidance.',
    inputSchema: {
      locationId: z.string().describe("Location ID (e.g., 'austin-tx') or partial name match"),
      specialty: z.enum(['pediatrics', 'cardiology', 'mental_health', 'maternity', 'geriatrics']).optional().describe('Optional specialty to get targeted guidance for'),
    },
    annotations: TOOL_ANNOTATIONS_READONLY,
    scope: 'read',
    handler: (svc, args, _userId) => {
      const loc = svc.relocation.getLocationById(args.locationId as string) as any;
      if (!loc) return { __error: `Location not found: ${args.locationId}` };
      ...
      const specialtyNote = args.specialty ? SPECIALTY_GUIDANCE[args.specialty as string] : null;
      return {
        location: { id: loc.id, name: loc.name, state: loc.state },
  ...
  },
  {
    ...
    description:
      'School quality overview for a location. Uses upward mobility data as a community-investment proxy and points the user to authoritative sources (GreatSchools, Niche, district sites).',
    inputSchema: {
      locationId: z.string().describe("Location ID (e.g., 'austin-tx') or partial name match"),
    },
    annotations: TOOL_ANNOTATIONS_READONLY,
    ...
    handler: (svc, args, _userId) => {
      ...
      const reconIndicator =
        m.mobilityPercentile >= 75 ? 'Strong' : m.mobilityPercentile >= 50 ? 'Average' : m.mobilityPercentile > 0 ? 'Below average' : 'No data';
  ...
  },
  {
    ...
    description:
      "Analyze how well a location fits a stated lifestyle — family_oriented, young_professional, retiree, outdoor_enthusiast, arts_culture, foodie.",
    inputSchema: {
      locationId: z.string().describe("Location ID (e.g., 'austin-tx') or partial name match"),
      lifestyle: z.enum(['family_oriented', 'young_professional', 'retiree', 'outdoor_enthusiast', 'arts_culture', 'foodie']),
    },
    annotations: TOOL_ANNOTATIONS_READONLY,
    scope: 'read',
    handler: (svc, args, _userId) => {
      const loc = svc.relocation.getLocationById(args.locationId as string) as any;
      if (!loc) return { __error: `Location not found: ${args.locationId}` };
      const lifestyle = args.lifestyle as string;
      const ho = loc.healthOutcomes;
  ...
  },
  {
    ...
    description:
      'Post-move settlement checklist for the first 30 days — healthcare, community, services, and (optionally) family + pet tasks.',
    inputSchema: {
      toLocationId: z.string().describe("Location ID of the new city/metro (e.g., 'austin-tx')"),
      ...
      employmentType: z.enum(['remote', 'hybrid', 'in_person', 'self_employed', 'unemployed']).optional().describe('Modifies services section (work setup, commute)'),
    },
    annotations: TOOL_ANNOTATIONS_READONLY,
    scope: 'read',
    handler: (svc, args, _userId) => {
      const loc = svc.relocation.getLocationById(args.toLocationId as string) as any;
      if (!loc) return { __error: `Location not found: ${args.toLocationId}` };
      const hc = loc.healthcare;
      const amen = loc.amenities;
      const trans = loc.transportation;
      ...
      if (args.employmentType) {
        const kind = args.employmentType as string;
        const empTasks: string[] = [];
        if (kind === 'remote') {
...
type TaskTemplate = {
  id: string; title: string; description: string; dueOffsetDays: number;
  category: 'research' | 'logistics' | 'admin' | 'housing' | 'financial'; phase: string;
  condition?: (o: TimelineOptions) => boolean;
};
interface TimelineOptions {
  familySize: number; hasChildren: boolean; hasPets: boolean;
...
const logisticsTools: ToolDefinition[] = [
  {
    ...
    description:
      "Generate a personalized moving checklist with phased tasks. Saves to the user's relocation journey.",
    inputSchema: {
      moveDate: z.string().describe('Target move date in ISO 8601 format (e.g., "2026-08-15")'),
      fromLocationId: z.string().describe("Origin location ID (e.g., 'chicago-il')"),
      toLocationId: z.string().describe("Destination location ID (e.g., 'austin-tx')"),
      familySize: z.number().int().min(1).default(1).describe('Number of people in household'),
      hasChildren: z.boolean().default(false).describe('Whether household includes children'),
      hasPets: z.boolean().default(false).describe('Whether household includes pets'),
      movingType: z.enum(['diy', 'professional', 'full_service']).default('professional').describe('Type of move'),
      save: z.boolean().default(true).describe('Persist timeline to user journey (default true)'),
    },
    annotations: TOOL_ANNOTATIONS_WRITE,
    scope: 'write',
    handler: (svc, args, userId) => {
      const moveDate = new Date(args.moveDate as string);
      if (Number.isNaN(moveDate.getTime())) {
        return { __error: 'Invalid moveDate. Use ISO 8601 (e.g., "2026-08-15").' };
      }
      const opts: TimelineOptions = {
        familySize: args.familySize as number,
        hasChildren: args.hasChildren as boolean,
        hasPets: args.hasPets as boolean,
        movingType: args.movingType as 'diy' | 'professional' | 'full_service',
      };
  ...
  },
  {
    ...
    description:
      'Estimate total moving costs with itemized breakdown and low/mid/high ranges.',
    inputSchema: {
      fromLocationId: z.string().describe("Origin location ID (e.g., 'chicago-il')"),
      toLocationId: z.string().describe("Destination location ID (e.g., 'austin-tx')"),
      homeSize: z.enum(['studio', '1br', '2br', '3br', '4br']).describe('Home size category'),
      movingType: z.enum(['diy', 'professional', 'full_service']).default('professional').describe('Type of move'),
      includeStorage: z.boolean().default(false).describe('Add estimated storage costs'),
      includeTempHousing: z.boolean().default(false).describe('Add estimated temporary housing'),
    },
    annotations: TOOL_ANNOTATIONS_READONLY,
    scope: 'read',
    handler: (svc, args, _userId) => {
      const from = svc.relocation.getLocationById(args.fromLocationId as string);
      const to = svc.relocation.getLocationById(args.toLocationId as string);
      if (!from) return { __error: `Unknown fromLocationId: ${args.fromLocationId}` };
      if (!to) return { __error: `Unknown toLocationId: ${args.toLocationId}` };

      const distanceMiles = Math.round(haversineMiles(from as any, to as any));
      const homeSize = args.homeSize as string;
      const movingType = args.movingType as string;
      const base = MOVING_BASE_COSTS[homeSize]?.[movingType];
      if (!base) return { __error: 'Invalid homeSize/movingType combo.' };
      ...
      const destRent = (to as any).cost?.medianRent ?? 1500;
      ...
      return {
        fromLocation: { id: (from as any).id, name: (from as any).name, state: (from as any).state },
        toLocation: { id: (to as any).id, name: (to as any).name, state: (to as any).state },
        distanceMiles,
        homeSize,
        movingType,
        breakdown: items,
  ...
  },
  {
    ...
    description:
      'Generate a utility setup/transfer checklist for the destination.',
    inputSchema: {
      toLocationId: z.string().describe("Destination location ID (e.g., 'austin-tx')"),
      homeType: z.enum(['apartment', 'house']).default('house').describe('Home type at destination'),
      startDate: z.string().optional().describe('Target move-in date (ISO). Defaults to 2 weeks out.'),
    },
    annotations: TOOL_ANNOTATIONS_READONLY,
    scope: 'read',
    handler: (svc, args, _userId) => {
      const to = svc.relocation.getLocationById(args.toLocationId as string);
      if (!to) return { __error: `Unknown toLocationId: ${args.toLocationId}` };
      const state = (to as any).state ?? '';
      const moveIn = args.startDate ? new Date(args.startDate as string) : new Date(Date.now() + 14 * 86400_000);
      ...
      return {
        toLocation: { id: (to as any).id, name: (to as any).name, state },
  ...
  },
  {
    ...
    description:
      "Mark a move timeline task as complete (or incomplete — calling again toggles). Persists to the user's relocation journey.",
    inputSchema: {
      taskId: z.string().describe("Task ID from the move timeline (e.g., 'get_quotes', 'transfer_utilities')"),
    },
    annotations: TOOL_ANNOTATIONS_WRITE,
    scope: 'write',
    handler: (svc, args, userId) => {
...
const adminTools: ToolDefinition[] = [
  {
    ...
    description:
      "State-specific driver's license and vehicle registration requirements for the destination state. Returns deadlines, required documents, fees, appointment info, REAL ID availability.",
    inputSchema: {
      toState: z.string().length(2).describe('Destination 2-letter state code (e.g., "TX", "FL", "CA")'),
    },
    annotations: TOOL_ANNOTATIONS_READONLY,
    scope: 'read',
    handler: (_svc, args, _userId) => {
      const code = (args.toState as string).toUpperCase();
      const d = (DMV_DATA as any)[code];
      if (!d) return { error: 'Unknown state code', toState: code, availableStates: Object.keys(DMV_DATA).sort() };
      return {
        toState: code,
        driverLicense: {
  ...
  },
  {
    ...
    description:
      "Voter registration rules for the destination state — registration deadline, online/mail/in-person availability, same-day registration, party affiliation rules.",
    inputSchema: {
      toState: z.string().length(2).describe('Destination 2-letter state code (e.g., "TX", "FL", "CA")'),
    },
    annotations: TOOL_ANNOTATIONS_READONLY,
    ...
    handler: (_svc, args, _userId) => {
      const code = (args.toState as string).toUpperCase();
      const v = (VOTER_DATA as any)[code];
      if (!v) return { error: 'Unknown state code', toState: code, availableStates: Object.keys(VOTER_DATA).sort() };
      const deadlineDesc =
        v.registrationDeadlineDaysBeforeElection === 0 ? 'Election Day registration is allowed'
  ...
  },
  {
    ...
    description:
      'How moving affects insurance across auto, home/renters, and health lines. Computes directional impact based on cost-of-living, property tax, and state insurance regulatory environment.',
    inputSchema: {
      fromLocationId: z.string().describe('Origin location ID (e.g., "san-francisco-ca")'),
      toLocationId: z.string().describe('Destination location ID (e.g., "austin-tx")'),
    },
    ...
    handler: (_svc, args, _userId) => {
      ...
      try { locations = loadLocations(); } catch { /* dataset missing */ }
  ...
  },
  {
    ...
    description:
      'Comprehensive universal address-change checklist — every entity to notify when you move.',
    inputSchema: {},
    annotations: TOOL_ANNOTATIONS_READONLY,
    scope: 'read',
...
const journeyTools: ToolDefinition[] = [
  {
    name: 'get_relocation_journey',
    description:
      "Get the user's complete relocation journey — shortlisted cities, saved comparisons, move timeline, preferences, completed tasks, and current phase.",
    inputSchema: {},
    annotations: TOOL_ANNOTATIONS_READONLY,
    scope: 'read',
  ...
  },
  {
    ...
    description:
      "Add a location to the user's relocation shortlist.",
    inputSchema: {
  ...
  },
  {
    ...
    description:
      "Update the user's relocation preferences (budget, household size, employment type, demographics, climate preference, priorities).",
    inputSchema: {
      ...
      priorities: z.record(z.string(), z.number()).optional().describe('Category → weight'),
    },
    annotations: TOOL_ANNOTATIONS_WRITE,
    scope: 'write',
  ...
  },
  {
    ...
    description:
      "Set the user's current phase in the relocation journey: discovery, housing, logistics, admin, or settlement.",
    inputSchema: {
...
export const ALL_RELOCATION_TOOLS: ToolDefinition[] = [
  ...relocationTools,
  ...costTools,
  ...communityTools,
  ...logisticsTools,
 ...
 */
export const coreTools = (): ToolDefinition[] => relocationTools;
export const costToolDefs = (): ToolDefinition[] => costTools;
export const communityToolDefs = (): ToolDefinition[] => communityTools;
export const logisticsToolDefs = (): ToolDefinition[] => logisticsTools;
export const adminToolDefs = (): ToolDefinition[] => adminTools;
export const journeyToolDefs = (): ToolDefinition[] => journeyTools;
