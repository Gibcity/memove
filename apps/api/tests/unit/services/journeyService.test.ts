...
describe('setPhotoProvider', () => {
  it('JOURNEY-SVC-072: sets provider info on an existing photo', () => {
    ...
    const updated = testDb.prepare(`
      ...
      FROM journey_photos jp JOIN memove_photos tkp ON tkp.id = jp.photo_id
      WHERE jp.id = ?
    `).get(photo!.id) as any;
...
describe('Edge cases', () => {
  ...
  it('JOURNEY-SVC-086: addTripToJourney syncs trip photos when present', () => {
    ...
    const photos = testDb.prepare(`
      ...
      JOIN memove_photos tkp ON tkp.id = jp.photo_id
      WHERE jp.journey_id = ?
    `).all(journey.id);
...
describe('addProviderPhoto — passphrase', () => {
  it('JOURNEY-SVC-088: addProviderPhoto with passphrase stores encrypted value on memove_photos', () => {
    const { user } = createUser(testDb);
    const journey = createJourney(testDb, user.id);
