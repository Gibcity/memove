...
describe('resolveOidcRole', () => {
  ...
  it('OIDC-SVC-011: returns admin when groups array contains OIDC_ADMIN_VALUE', () => {
    process.env.OIDC_ADMIN_VALUE = 'memove-admins';
    expect(resolveOidcRole({ sub: 'x', groups: ['memove-users', 'memove-admins'] }, false)).toBe('admin');
  });

  it('OIDC-SVC-012: returns user when groups array does not contain OIDC_ADMIN_VALUE', () => {
    process.env.OIDC_ADMIN_VALUE = 'memove-admins';
    expect(resolveOidcRole({ sub: 'x', groups: ['memove-users'] }, false)).toBe('user');
  });
...
describe('verifyIdToken', () => {
  ...
  const ISSUER = 'https://auth.example.com/application/o/memove';
  const CLIENT_ID = 'memove-client';
