...
exports.safeFetchFollow = safeFetchFollow;
...
function isAlwaysBlocked(ip) {
    ...
    if (addr.startsWith('169.254.') || /^fe80:/i.test(addr))
        return true;
    // IPv4-mapped loopback / link-local: ::ffff:127.x.x.x, ::ffff:169.254.x.x
    if (/^::ffff:127\./i.test(addr) || /^::ffff:169\.254\./i.test(addr))
        return true;
    return false;
}
...
function isPrivateNetwork(ip) {
    ...
    if (/^::ffff:10\./i.test(addr))
        return true;
    if (/^::ffff:172\.(1[6-9]|2\d|3[01])\./i.test(addr))
        return true;
    if (/^::ffff:192\.168\./i.test(addr))
        return true;
    return false;
}
...
async function checkSsrf(rawUrl, bypassInternalIpAllowed = false) {
    let url;
    try {
        url = new URL(rawUrl);
    }
 ...
 */
async function safeFetchFollow(url, init, options) {
 ...
 */
function createPinnedDispatcher(resolvedIp, rejectUnauthorized = true) {
