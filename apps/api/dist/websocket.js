...
const ephemeralTokens_1 = require("./services/ephemeralTokens");
...
const socketMsgCounts = new WeakMap();
...
function setupWebSocket(server) {
    ...
    wss.on('connection', (ws, req) => {
        ...
        const consumed = (0, ephemeralTokens_1.consumeEphemeralTokenWithMeta)(token, 'ws');
        if (!consumed) {
        ...
        let row;
        row = database_1.db.prepare('SELECT id, username, email, role, mfa_enabled, password_version FROM users WHERE id = ?').get(userId);
        if (!row) {
