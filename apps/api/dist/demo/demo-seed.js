...
function seedDemoData(db) {
    ...
    const renameOrDrop = (oldEmail, newEmail) => {
    ...
    const e2eHash = bcryptjs_1.default.hashSync(E2E_PASS, 10);
    const existingE2E = db.prepare('SELECT id, password_hash, must_change_password FROM users WHERE email = ?').get(E2E_EMAIL);
    if (existingE2E) {
