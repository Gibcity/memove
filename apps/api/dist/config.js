...
exports.SESSION_DURATION_REMEMBER_SECONDS = exports.SESSION_DURATION_REMEMBER_MS = exports.SESSION_DURATION_REMEMBER = exports.SESSION_DURATION_SECONDS = exports.SESSION_DURATION_MS = exports.SESSION_DURATION = exports.DEFAULT_LANGUAGE = exports.JWT_SECRET = exports.ENCRYPTION_KEY = void 0;
exports.updateJwtSecret = updateJwtSecret;
...
const jwtSecretFile = node_path_1.default.join(dataDir, '.jwt_secret');
...
//      data/.encryption_key.
const encKeyFile = node_path_1.default.join(dataDir, '.encryption_key');
let _encryptionKey = process.env.ENCRYPTION_KEY || '';
if (_encryptionKey) {
    ...
    try {
        ...
        node_fs_1.default.writeFileSync(encKeyFile, _encryptionKey, { mode: 0o600 });
    }
    catch {
    ...
    try {
        _encryptionKey = node_fs_1.default.readFileSync(encKeyFile, 'utf8').trim();
    }
    catch {
    ...
    if (!_encryptionKey) {
        ...
        try {
            _encryptionKey = node_fs_1.default.readFileSync(jwtSecretFile, 'utf8').trim();
    ...
    if (!_encryptionKey) {
        ...
        _encryptionKey = node_crypto_1.default.randomBytes(32).toString('hex');
    }
...
exports.ENCRYPTION_KEY = _encryptionKey;
...
let _jwtSecret;
try {
    _jwtSecret = node_fs_1.default.readFileSync(jwtSecretFile, 'utf8').trim();
}
catch {
    _jwtSecret = node_crypto_1.default.randomBytes(32).toString('hex');
    try {
        if (!node_fs_1.default.existsSync(dataDir))
            node_fs_1.default.mkdirSync(dataDir, { recursive: true });
        node_fs_1.default.writeFileSync(jwtSecretFile, _jwtSecret, { mode: 0o600 });
...
if (!shared_1.SUPPORTED_LANGUAGE_CODES.includes(rawDefaultLang)) {
    console.warn(`DEFAULT_LANGUAGE="${rawDefaultLang}" is not supported. Falling back to "en". Supported: ${shared_1.SUPPORTED_LANGUAGE_CODES.join(', ')}`);
}
exports.DEFAULT_LANGUAGE = shared_1.SUPPORTED_LANGUAGE_CODES.includes(rawDefaultLang) ? rawDefaultLang : 'en';
// SESSION_DURATION controls how long a memove session (the `memove_session` JWT
...
exports.SESSION_DURATION = parsedSessionMs == null ? DEFAULT_SESSION_DURATION : rawSessionDuration;
/** Session length in milliseconds — used for the cookie `maxAge`. */
exports.SESSION_DURATION_MS = parsedSessionMs ?? parseDurationMs(DEFAULT_SESSION_DURATION);
/** Session length in seconds — passed to `jwt.sign({ expiresIn })` (number = seconds). */
exports.SESSION_DURATION_SECONDS = Math.floor(exports.SESSION_DURATION_MS / 1000);
...
// persistent `memove_session` cookie `maxAge`. An unticked login keeps
...
exports.SESSION_DURATION_REMEMBER = parsedRememberMs == null ? DEFAULT_SESSION_DURATION_REMEMBER : rawRememberDuration;
/** "Remember me" session length in milliseconds — used for the persistent cookie `maxAge`. */
exports.SESSION_DURATION_REMEMBER_MS = parsedRememberMs ?? parseDurationMs(DEFAULT_SESSION_DURATION_REMEMBER);
/** "Remember me" session length in seconds — passed to `jwt.sign({ expiresIn })`. */
exports.SESSION_DURATION_REMEMBER_SECONDS = Math.floor(exports.SESSION_DURATION_REMEMBER_MS / 1000);
