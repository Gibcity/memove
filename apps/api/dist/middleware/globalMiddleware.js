...
const mfaPolicy_1 = require("./mfaPolicy");
/**
 ...
 */
function applyGlobalMiddleware(app, opts = {}) {
    ...
    app.use((req, _res, next) => {
        if (req.path.startsWith('/.well-known/') ||
            ...
            req.path === '/mcp') {
            (0, cors_1.default)({ origin: '*', credentials: false })(req, _res, next);
        }
        else {
            next();
        }
    ...
    app.use((req, res, next) => {
        ...
        res.on('finish', () => {
            ...
            if (res.statusCode >= 500) {
                (0, auditLog_1.logError)(`${req.method} ${req.path} ${res.statusCode} ${ms}ms ip=${req.ip}`);
            }
            else if (res.statusCode === 401 || res.statusCode === 403) {
                (0, auditLog_1.logDebug)(`${req.method} ${req.path} ${res.statusCode} ${ms}ms ip=${req.ip}`);
            }
            else if (res.statusCode >= 400) {
                (0, auditLog_1.logWarn)(`${req.method} ${req.path} ${res.statusCode} ${ms}ms ip=${req.ip}`);
            }
            ...
            (0, auditLog_1.logDebug)(`${req.method} ${req.path} ${res.statusCode} ${ms}ms ip=${req.ip}${q}${b}`);
        });
        next();
    });
}
