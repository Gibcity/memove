...
exports.applyIdempotency = applyIdempotency;
 ...
 */
function applyIdempotency(req, res, next, userId) {
    ...
    const existing = database_1.db.prepare('SELECT status_code, response_body FROM idempotency_keys WHERE key = ? AND user_id = ? AND method = ? AND path = ?').get(key, userId, req.method, req.path);
    if (existing) {
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
            try {
                ...
                if (serialized.length <= MAX_CACHED_BODY_BYTES) {
                    database_1.db.prepare(`INSERT OR IGNORE INTO idempotency_keys (key, user_id, method, path, status_code, response_body, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?)`).run(key, userId, req.method, req.path, res.statusCode, serialized, Math.floor(Date.now() / 1000));
                }
            }
            catch {
