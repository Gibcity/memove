...
exports.getDetailedWeather = getDetailedWeather;
// ── WMO code mappings ───────────────────────────────────────────────────
const WMO_MAP = {
...
// ── getWeather ──────────────────────────────────────────────────────────
async function _getWeatherImpl(lat, lng, date, lang) {
    ...
    if (date) {
        ...
        if (diffDays > -1) {
            ...
            if (new Date(refYear, month - 1, day + 2) > yesterday)
                refYear--;
...
async function getWeather(lat, lng, date, lang) {
    const ck = cacheKey(lat, lng, date);
    const cached = getCached(ck);
    if (cached)
        return cached;
...
// ── getDetailedWeather ──────────────────────────────────────────────────
async function _getDetailedWeatherImpl(lat, lng, date, lang) {
    ...
    if (diffDays > 16) {
        ...
        // Archive API only has data up to yesterday — go back further if needed
        const yesterday = new Date(now.getTime() - 86400000);
        if (new Date(refYear, targetDate.getMonth(), targetDate.getDate()) > yesterday)
            refYear--;
...
async function getDetailedWeather(lat, lng, date, lang) {
    const ck = `detailed_${cacheKey(lat, lng, date)}`;
    const cached = getCached(ck);
    if (cached)
        return cached;
