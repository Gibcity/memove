...
exports.resolveWebauthnConfig = resolveWebauthnConfig;
exports.isPasskeyConfigured = isPasskeyConfigured;
...
function getSetting(key) {
    const raw = database_1.db.prepare('SELECT value FROM app_settings WHERE key = ?').get(key)?.value;
...
function isIpHost(host) {
    if (/^\d{1,3}(\.\d{1,3}){3}$/.test(host))
        return true; // IPv4
    if (host.includes(':'))
        return true; // IPv6 (hostname keeps the colons)
    return false;
}
function resolveWebauthnConfig() {
    ...
    if (!rpID)
        return null; // bare IP / unresolved → WebAuthn cannot be used here
    ...
    if (origins.length === 0)
        return null;
    return { rpID, rpName: 'memove', origins };
}
