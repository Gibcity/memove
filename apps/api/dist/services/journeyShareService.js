...
exports.createOrUpdateJourneyShareLink = createOrUpdateJourneyShareLink;
exports.getJourneyShareLink = getJourneyShareLink;
exports.deleteJourneyShareLink = deleteJourneyShareLink;
exports.validateShareTokenForPhoto = validateShareTokenForPhoto;
exports.validateShareTokenForAsset = validateShareTokenForAsset;
exports.getPublicJourney = getPublicJourney;
const database_1 = require("../db/database");
const crypto_1 = __importDefault(require("crypto"));
const journeyService_1 = require("./journeyService");
function createOrUpdateJourneyShareLink(journeyId, createdBy, permissions) {
    ...
    if (!(0, journeyService_1.isOwner)(journeyId, createdBy))
        return null;
    const { share_timeline = true, share_gallery = true, share_map = true, } = permissions;
    const existing = database_1.db.prepare('SELECT token FROM journey_share_tokens WHERE journey_id = ?').get(journeyId);
    if (existing) {
        database_1.db.prepare('UPDATE journey_share_tokens SET share_timeline = ?, share_gallery = ?, share_map = ? WHERE journey_id = ?')
    ...
    database_1.db.prepare('INSERT INTO journey_share_tokens (journey_id, token, created_by, share_timeline, share_gallery, share_map) VALUES (?, ?, ?, ?, ?, ?)')
        .run(journeyId, token, createdBy, share_timeline ? 1 : 0, share_gallery ? 1 : 0, share_map ? 1 : 0);
    return { token, created: true };
}
function getJourneyShareLink(journeyId) {
    const row = database_1.db.prepare('SELECT * FROM journey_share_tokens WHERE journey_id = ?').get(journeyId);
    if (!row)
        return null;
    return {
        token: row.token,
...
function deleteJourneyShareLink(journeyId, userId) {
    if (!(0, journeyService_1.isOwner)(journeyId, userId))
        return false;
    database_1.db.prepare('DELETE FROM journey_share_tokens WHERE journey_id = ?').run(journeyId);
    return true;
}
function validateShareTokenForPhoto(token, photoId) {
    const row = database_1.db.prepare('SELECT journey_id FROM journey_share_tokens WHERE token = ?').get(token);
    if (!row)
        return null;
  ...
  `).get(photoId, row.journey_id);
    if (!photo)
        return null;
    const journey = database_1.db.prepare('SELECT user_id FROM journeys WHERE id = ?').get(row.journey_id);
    return journey ? { journeyId: row.journey_id, ownerId: photo.owner_id || journey.user_id } : null;
}
function validateShareTokenForAsset(token, assetId) {
    const row = database_1.db.prepare('SELECT journey_id FROM journey_share_tokens WHERE token = ?').get(token);
    if (!row)
        return null;
    const photo = database_1.db.prepare(`
  ...
  `).get(assetId, row.journey_id);
    // Only resolve assets that actually belong to this shared journey.
    if (!photo)
        return null;
    return { ownerId: photo.owner_id };
}
function getPublicJourney(token) {
    const row = database_1.db.prepare('SELECT * FROM journey_share_tokens WHERE token = ?').get(token);
    if (!row)
        return null;
    const journey = database_1.db.prepare('SELECT * FROM journeys WHERE id = ?').get(row.journey_id);
    if (!journey)
        return null;
    // Entries with photos
