...
exports.mapFlightToReservation = mapFlightToReservation;
...
function localParts(iso, tz) {
    ...
    try {
        ...
        let hh = get('hour');
        if (hh === '24')
            hh = '00'; // some ICU builds emit 24:00 for midnight
...
function mapFlightToReservation(raw) {
    ...
    const metadata = {};
    if (airlineCode)
        metadata.airline = airlineCode;
    if (raw.flightNumber)
        metadata.flight_number = raw.flightNumber;
    if (aircraftCode)
        metadata.aircraft = aircraftCode;
    if (raw.aircraftReg)
        metadata.aircraft_reg = raw.aircraftReg;
    if (raw.flightReason)
        metadata.flight_reason = raw.flightReason;
    if (seat?.seatNumber)
        metadata.seat = seat.seatNumber;
