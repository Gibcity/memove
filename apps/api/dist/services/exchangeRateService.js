...
exports.convertWithRates = convertWithRates;
...
async function fetchRates(base) {
    try {
        ...
        if (!Array.isArray(data))
            return null;
        const rates = { [base.toUpperCase()]: 1 };
        for (const r of data) {
            if (r && typeof r.quote === 'string' && typeof r.rate === 'number')
...
async function getRates(base) {
    ...
    if (!p) {
        p = fetchRates(key).then(rates => {
            if (rates)
                cache.set(key, { rates, ts: Date.now() });
            inflight.delete(key);
            return rates;
        });
        inflight.set(key, p);
 ...
 */
function convertWithRates(amount, from, base, rates) {
    const fromCur = (from || base).toUpperCase();
    const baseCur = base.toUpperCase();
    if (fromCur === baseCur || !rates)
        return amount;
