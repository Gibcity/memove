...
exports.NAME_TO_CODE = exports.COUNTRY_BOXES = void 0;
exports.getCountryGeo = getCountryGeo;
exports.getRegionGeo = getRegionGeo;
exports.getCached = getCached;
exports.setCache = setCache;
exports.reverseGeocodeCountry = reverseGeocodeCountry;
exports.getCountryFromCoords = getCountryFromCoords;
exports.getCountryFromAddress = getCountryFromAddress;
exports.getStats = getStats;
exports.getCountryPlaces = getCountryPlaces;
exports.listVisitedCountries = listVisitedCountries;
exports.markCountryVisited = markCountryVisited;
exports.unmarkCountryVisited = unmarkCountryVisited;
exports.listManuallyVisitedRegions = listManuallyVisitedRegions;
exports.markRegionVisited = markRegionVisited;
exports.unmarkRegionVisited = unmarkRegionVisited;
exports.getVisitedRegions = getVisitedRegions;
exports.listBucketList = listBucketList;
exports.createBucketItem = createBucketItem;
exports.updateBucketItem = updateBucketItem;
exports.deleteBucketItem = deleteBucketItem;
...
const geoBundleCache = new Map();
function loadGeoBundle(name) {
    const cached = geoBundleCache.get(name);
    if (cached)
        return cached;
    const file = path_1.default.join(__dirname, '..', '..', 'assets', 'atlas', `${name}.geojson.gz`);
    ...
    const geo = JSON.parse(zlib_1.default.gunzipSync(fs_1.default.readFileSync(file)).toString('utf8'));
...
function getCountryGeo() {
    return loadGeoBundle('admin0');
}
async function getRegionGeo(countryCodes) {
...
function setCache(lat, lng, code) {
...
async function reverseGeocodeCountry(lat, lng) {
    ...
    try {
        const res = await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json&zoom=3&accept-language=en`, {
            headers: { 'User-Agent': 'memove (https://github.com/Gibcity/memove)' },
...
function getCountryFromCoords(lat, lng) {
    ...
    for (const [code, [minLng, minLat, maxLng, maxLat]] of Object.entries(exports.COUNTRY_BOXES)) {
...
function getCountryFromAddress(address) {
    ...
    if (exports.NAME_TO_CODE[normalized])
        return exports.NAME_TO_CODE[normalized];
    if (exports.NAME_TO_CODE[last])
        return exports.NAME_TO_CODE[last];
    if (last.length === 2 && last === last.toUpperCase())
        return last;
    return null;
}
// ── Resolve a place to a country code (address -> bbox -> geocode) ──────────
async function resolveCountryCode(place) {
...
function resolveCountryCodeSync(place) {
    let code = getCountryFromAddress(place.address);
    if (!code && place.lat && place.lng) {
        code = getCountryFromCoords(place.lat, place.lng);
    }
...
function getPlacesForTrips(tripIds) {
    if (tripIds.length === 0)
        return [];
    const placeholders = tripIds.map(() => '?').join(',');
    return database_1.db.prepare(`SELECT * FROM places WHERE trip_id IN (${placeholders})`).all(...tripIds);
}
// ── Country resolution (batch DB cache + sync fallback + background geocoding) ──
function resolvePlaceCountries(places) {
    const out = new Map();
    const geoPlaces = places.filter(p => p.lat && p.lng);
    const placeIds = geoPlaces.map(p => p.id);
    const cached = placeIds.length > 0
        ? database_1.db.prepare(`SELECT place_id, country_code FROM place_regions WHERE place_id IN (${placeIds.map(() => '?').join(',')})`).all(...placeIds)
        : [];
    const cachedMap = new Map(cached.map(r => [r.place_id, r.country_code]));
    const uncachedForGeocode = [];
    ...
    if (uncachedForGeocode.length > 0) {
        const insertStmt = database_1.db.prepare('INSERT OR REPLACE INTO place_regions (place_id, country_code, region_code, region_name) VALUES (?, ?, ?, ?)');
        for (const p of uncachedForGeocode)
            geocodingInFlight.add(p.id);
        void (async () => {
            try {
...
async function getStats(userId) {
    ...
    const continents = {};
    countries.forEach(c => {
        const cont = shared_1.CONTINENT_MAP[c.code] || 'Other';
...
// ── getCountryPlaces ────────────────────────────────────────────────────────
function getCountryPlaces(userId, code) {
    const trips = getUserTrips(userId);
    const tripIds = trips.map(t => t.id);
    if (tripIds.length === 0)
        return { places: [], trips: [], manually_marked: false };
    const places = getPlacesForTrips(tripIds);
...
// ── Mark / unmark country ───────────────────────────────────────────────────
function listVisitedCountries(userId) {
    return database_1.db.prepare('SELECT country_code, created_at FROM visited_countries WHERE user_id = ? ORDER BY created_at DESC').all(userId);
}
function markCountryVisited(userId, code) {
    database_1.db.prepare('INSERT OR IGNORE INTO visited_countries (user_id, country_code) VALUES (?, ?)').run(userId, code);
}
function unmarkCountryVisited(userId, code) {
    database_1.db.prepare('DELETE FROM visited_countries WHERE user_id = ? AND country_code = ?').run(userId, code);
    database_1.db.prepare('DELETE FROM visited_regions WHERE user_id = ? AND country_code = ?').run(userId, code);
}
// ── Mark / unmark region ────────────────────────────────────────────────────
function listManuallyVisitedRegions(userId) {
    return database_1.db.prepare('SELECT region_code, region_name, country_code FROM visited_regions WHERE user_id = ? ORDER BY created_at DESC').all(userId);
}
function markRegionVisited(userId, regionCode, regionName, countryCode) {
    database_1.db.prepare('INSERT OR IGNORE INTO visited_regions (user_id, region_code, region_name, country_code) VALUES (?, ?, ?, ?)').run(userId, regionCode, regionName, countryCode);
    // Auto-mark parent country if not already visited
    database_1.db.prepare('INSERT OR IGNORE INTO visited_countries (user_id, country_code) VALUES (?, ?)').run(userId, countryCode);
}
function unmarkRegionVisited(userId, regionCode) {
    const region = database_1.db.prepare('SELECT country_code FROM visited_regions WHERE user_id = ? AND region_code = ?').get(userId, regionCode);
    database_1.db.prepare('DELETE FROM visited_regions WHERE user_id = ? AND region_code = ?').run(userId, regionCode);
    if (region) {
        const remaining = database_1.db.prepare('SELECT COUNT(*) as count FROM visited_regions WHERE user_id = ? AND country_code = ?').get(userId, region.country_code);
        if (remaining.count === 0) {
            database_1.db.prepare('DELETE FROM visited_countries WHERE user_id = ? AND country_code = ?').run(userId, region.country_code);
        }
    }
}
...
async function fetchNominatimAddress(lat, lng, zoom) {
    await throttleNominatim();
    try {
        const res = await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json&zoom=${zoom}&accept-language=en`, {
            headers: { 'User-Agent': 'memove (https://github.com/Gibcity/memove)' },
            signal: AbortSignal.timeout(10_000),
        });
        if (!res.ok)
...
function buildRegionInfo(address, preferFinest) {
...
async function reverseGeocodeRegion(lat, lng) {
    ...
    if (!address)
        return null; // transient failure — leave uncached so a later call retries
    ...
    if (info && info.country_code === 'GB' && GB_CONSTITUENT_CODES.has(info.region_code)) {
        ...
        if (finer && !GB_CONSTITUENT_CODES.has(finer.region_code))
            info = finer;
    }
    regionCache.set(key, info);
    return info;
}
async function getVisitedRegions(userId) {
    const trips = getUserTrips(userId);
    const tripIds = trips.map(t => t.id);
    const places = getPlacesForTrips(tripIds);
    ...
    const cached = placeIds.length > 0
        ? database_1.db.prepare(`SELECT * FROM place_regions WHERE place_id IN (${placeIds.map(() => '?').join(',')})`).all(...placeIds)
        : [];
    ...
    for (const [, entry] of cachedMap) {
        if (!regionMap[entry.country_code])
            regionMap[entry.country_code] = new Map();
...
// ── Bucket list CRUD ────────────────────────────────────────────────────────
function listBucketList(userId) {
    return database_1.db.prepare('SELECT * FROM bucket_list WHERE user_id = ? ORDER BY created_at DESC').all(userId);
}
function createBucketItem(userId, data) {
    const result = database_1.db.prepare('INSERT INTO bucket_list (user_id, name, lat, lng, country_code, notes, target_date) VALUES (?, ?, ?, ?, ?, ?, ?)').run(userId, data.name.trim(), data.lat ?? null, data.lng ?? null, data.country_code ?? null, data.notes ?? null, data.target_date ?? null);
    return database_1.db.prepare('SELECT * FROM bucket_list WHERE id = ?').get(result.lastInsertRowid);
}
function updateBucketItem(userId, itemId, data) {
    const item = database_1.db.prepare('SELECT * FROM bucket_list WHERE id = ? AND user_id = ?').get(itemId, userId);
    if (!item)
        return null;
    ...
    WHERE id = ?`).run(data.name?.trim() || null, data.notes !== undefined ? 1 : 0, data.notes !== undefined ? (data.notes || null) : null, data.lat !== undefined ? 1 : 0, data.lat !== undefined ? (data.lat || null) : null, data.lng !== undefined ? 1 : 0, data.lng !== undefined ? (data.lng || null) : null, data.country_code !== undefined ? 1 : 0, data.country_code !== undefined ? (data.country_code || null) : null, data.target_date !== undefined ? 1 : 0, data.target_date !== undefined ? (data.target_date || null) : null, itemId);
    return database_1.db.prepare('SELECT * FROM bucket_list WHERE id = ?').get(itemId);
}
function deleteBucketItem(userId, itemId) {
    const item = database_1.db.prepare('SELECT * FROM bucket_list WHERE id = ? AND user_id = ?').get(itemId, userId);
    if (!item)
        return false;
