...
function seedAdminAccount(db) {
    try {
        ...
        if (env_admin_email && env_admin_pw) {
            ...
            password = crypto_1.default.randomBytes(12).toString('base64url');
...
function seedAddons(db) {
    ...
    `);
        ...
        for (const p of providerRows)
            insertProvider.run(p.id, p.name, p.description, p.icon, p.enabled, p.sort_order);
