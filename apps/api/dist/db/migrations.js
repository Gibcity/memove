...
exports.trimUserWhitespace = trimUserWhitespace;
exports.runMigrations = runMigrations;
const apiKeyCrypto_1 = require("../services/apiKeyCrypto");
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const zlib_1 = __importDefault(require("zlib"));
/** Returns true if any collision was encountered (renamed row). */
function trimUserWhitespace(db) {
    ...
    for (const row of dirtyUsernames) {
        ...
        if (collision) {
            hadCollision = true;
            console.warn(`[migration] WHITESPACE COLLISION username: user id=${row.id} ` +
                `original=${JSON.stringify(row.username)} trimmed="${trimmed}" ` +
                `collides with user id=${collision.id}. Renamed to "${final}". ` +
                `Manual review required.`);
        }
        else {
            console.warn(`[migration] Trimmed username for user id=${row.id}: ` + `${JSON.stringify(row.username)} → "${final}"`);
        }
        db.prepare(`UPDATE users SET username = ? WHERE id = ?`).run(final, row.id);
    }
    const dirtyEmails = db.prepare(`SELECT id, email FROM users WHERE email != TRIM(email)`).all();
    for (const row of dirtyEmails) {
        ...
        if (collision) {
            ...
            console.warn(`[migration] WHITESPACE COLLISION email: user id=${row.id} ` +
...
function runMigrations(db) {
    ...
    `),
        ...
        () => {
            try {
                db.prepare('INSERT OR IGNORE INTO addons (id, name, description, type, icon, enabled, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)').run('mcp', 'MCP', 'Model Context Protocol for AI assistant integration', 'integration', 'Terminal', 0, 12);
            }
            catch (err) {
                console.warn('[migrations] Non-fatal migration step failed:', err);
            }
    ...
    `),
        ...
        () => {
            const row = db.prepare("SELECT value FROM app_settings WHERE key = 'oidc_client_secret'").get();
            if (row?.value && !row.value.startsWith('enc:v1:')) {
                db.prepare("UPDATE app_settings SET value = ? WHERE key = 'oidc_client_secret'").run((0, apiKeyCrypto_1.encrypt_api_key)(row.value));
            }
        },
        // Encrypt any plaintext smtp_pass left in app_settings
        () => {
            const row = db.prepare("SELECT value FROM app_settings WHERE key = 'smtp_pass'").get();
            if (row?.value && !row.value.startsWith('enc:v1:')) {
                db.prepare("UPDATE app_settings SET value = ? WHERE key = 'smtp_pass'").run((0, apiKeyCrypto_1.encrypt_api_key)(row.value));
            }
        },
        // Encrypt any plaintext immich_api_key values in the users table
        () => {
            const rows = db
                .prepare("SELECT id, immich_api_key FROM users WHERE immich_api_key IS NOT NULL AND immich_api_key != '' AND immich_api_key NOT LIKE 'enc:v1:%'")
                .all();
            for (const row of rows) {
                db.prepare('UPDATE users SET immich_api_key = ? WHERE id = ?').run((0, apiKeyCrypto_1.encrypt_api_key)(row.immich_api_key), row.id);
            }
        },
        () => {
            try {
      ...
      `);
        ...
        `);
                ...
                const columns = db.prepare("PRAGMA table_info('trip_photos')").all();
        ...
        `);
                ...
                const columns = db.prepare("PRAGMA table_info('trip_album_links')").all();
                const names = new Set(columns.map((c) => c.name));
        ...
        `).run('synologyphotos', 'Synology Photos', 'Synology Photos integration with separate account settings', 'Image', 0, 1);
            }
            catch (err) {
        ...
        () => {
            ...
            const columns = db.prepare("PRAGMA table_info('photo_providers')").all();
            const names = new Set(columns.map((c) => c.name));
            if (!names.has('config'))
                return;
        ...
        () => {
            const columns = db.prepare("PRAGMA table_info('trip_photos')").all();
            const names = new Set(columns.map((c) => c.name));
            if (names.has('asset_id') && !names.has('immich_asset_id'))
                return;
        ...
        () => {
            ...
            try {
                db.exec('ALTER TABLE trip_photos ADD COLUMN album_link_id INTEGER REFERENCES trip_album_links(id) ON DELETE SET NULL DEFAULT NULL');
            }
            catch (err) {
                if (!err.message?.includes('duplicate column name'))
                    throw err;
      ...
      `);
            ...
            const tableExists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='notification_preferences'").get() != null;
            const oldPrefs = tableExists
                ? db.prepare('SELECT * FROM notification_preferences').all()
                : [];
            const eventCols = {
            ...
            const insert = db.prepare('INSERT OR IGNORE INTO notification_channel_preferences (user_id, event_type, channel, enabled) VALUES (?, ?, ?, ?)');
            ...
            for (const row of oldPrefs) {
                ...
                for (const [eventType, col] of Object.entries(eventCols)) {
                    ...
                    if (!emailEnabled)
                        rows.push([userId, eventType, 'email', 0]);
                    if (!webhookEnabled)
                        rows.push([userId, eventType, 'webhook', 0]);
                }
                if (rows.length > 0)
                    insertMany(rows);
            }
      ...
      `);
        ...
        () => {
            try {
                db.exec('ALTER TABLE budget_items ADD COLUMN reservation_id INTEGER REFERENCES reservations(id) ON DELETE SET NULL DEFAULT NULL');
            }
            catch (err) {
                if (!err.message?.includes('duplicate column name'))
                    throw err;
        ...
        () => {
            ...
            try {
                db.exec('ALTER TABLE packing_bags ADD COLUMN user_id INTEGER REFERENCES users(id) ON DELETE SET NULL DEFAULT NULL');
            }
            catch (err) {
                if (!err.message?.includes('duplicate column name'))
                    throw err;
      ...
      `);
            ...
            const bagsWithUser = db.prepare('SELECT id, user_id FROM packing_bags WHERE user_id IS NOT NULL').all();
            const ins = db.prepare('INSERT OR IGNORE INTO packing_bag_members (bag_id, user_id) VALUES (?, ?)');
            for (const b of bagsWithUser)
                ins.run(b.id, b.user_id);
        },
      ...
      `);
            ...
            const reservations = db
                .prepare('SELECT id, trip_id, reservation_time, reservation_end_time, day_plan_position FROM reservations WHERE day_plan_position IS NOT NULL')
                .all();
            ...
            for (const r of reservations) {
                ...
                const matchingDays = db
                    ...
                    .all(r.trip_id, startDate, endDate);
                for (const d of matchingDays)
                    ins.run(r.id, d.id, r.day_plan_position);
            }
        },
      ...
      `);
        ...
        () => {
            const cols = db.prepare('PRAGMA table_info(photo_provider_fields)').all();
      ...
      `);
            ...
            VALUES (?, ?, ?, ?, ?, ?)
          `)
                        .run(j.user_id, j.title || 'Untitled Journey', j.description || null, j.status || 'draft', j.created_at ? new Date(j.created_at).getTime() : ts, j.updated_at ? new Date(j.updated_at).getTime() : ts);
          ...
          `).run(Number(res.lastInsertRowid), j.user_id, ts);
            ...
            VALUES (?, ?, 'entry', ?, ?, ?, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `)
                        .run(newJourneyId, e.user_id || oldJourneys.find((j) => j.id === e.journey_id)?.user_id || 1, e.title || null, e.body || null, e.entry_date || new Date().toISOString().split('T')[0], e.place_name || null, e.lat || null, e.lng || null, e.mood || null, e.weather || null, e.visibility || 'private', e.sort_order || 0, e.created_at ? new Date(e.created_at).getTime() : ts, e.updated_at ? new Date(e.updated_at).getTime() : ts);
                ...
                for (const p of oldPhotos) {
                    ...
                    if (!newEntryId || !p.file_path)
                        continue;
                    db.prepare(`
            INSERT INTO journey_photos (entry_id, file_path, thumbnail_path, caption, sort_order, width, height, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
          `).run(newEntryId, p.file_path, p.thumbnail_path || null, p.caption || null, p.sort_order || 0, p.width || null, p.height || null, p.created_at ? new Date(p.created_at).getTime() : ts);
                }
      ...
      `);
        ...
        // Central memove_photos registry; trip_photos + journey_photos reference via photo_id
        () => {
            // 1. Create the central photo registry
            db.exec(`
        CREATE TABLE IF NOT EXISTS memove_photos (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          provider TEXT NOT NULL,
          asset_id TEXT,
      ...
      `);
            db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_memove_photos_provider_asset ON memove_photos(provider, asset_id, owner_id) WHERE asset_id IS NOT NULL');
            db.exec('CREATE INDEX IF NOT EXISTS idx_memove_photos_owner ON memove_photos(owner_id)');
            // 2. Migrate trip_photos → memove_photos + photo_id FK
            const tripPhotosExists = db
                .prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'trip_photos'")
                .get();
            if (tripPhotosExists) {
                // Detect schema variant: old (immich_asset_id) vs new (asset_id + provider)
                const tpCols = db.prepare("PRAGMA table_info('trip_photos')").all();
                ...
                if (assetCol) {
                    ...
                    // Qualified alias needed in JOIN context where both trip_photos and memove_photos have provider
                    ...
                    // Insert existing trip photo references into memove_photos
                    db.exec(`
            INSERT OR IGNORE INTO memove_photos (provider, asset_id, owner_id, created_at)
          ...
          `);
            ...
            CREATE TABLE trip_photos_new (
              ...
              trip_id INTEGER NOT NULL REFERENCES trips(id) ON DELETE CASCADE,
              user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
              photo_id INTEGER NOT NULL REFERENCES memove_photos(id) ON DELETE CASCADE,
              shared INTEGER NOT NULL DEFAULT 1,
          ...
          `);
            ...
            FROM trip_photos tp
            JOIN memove_photos tkp ON tkp.provider = ${providerJoinExpr} AND tkp.asset_id = tp.${assetCol} AND tkp.owner_id = tp.user_id
          `);
                }
                else {
          ...
          `);
            ...
            // 3. Migrate journey_photos → memove_photos + photo_id FK
            ...
            if (journeyPhotosExists) {
                // Insert provider-based journey photos into memove_photos
                db.exec(`
          INSERT OR IGNORE INTO memove_photos (provider, asset_id, owner_id, width, height, created_at)
        ...
        `);
                // Insert local journey photos into memove_photos (each is unique)
                db.exec(`
          INSERT INTO memove_photos (provider, file_path, thumbnail_path, width, height, created_at)
        ...
        `);
          ...
          JOIN memove_photos tkp ON tkp.provider = jp.provider AND tkp.asset_id = jp.asset_id AND tkp.owner_id = jp.owner_id
        ...
        `);
          ...
          SELECT jp.entry_id, tkp.id, jp.caption, jp.sort_order, jp.shared, jp.created_at
          FROM journey_photos jp
          JOIN memove_photos tkp ON tkp.provider = 'local' AND tkp.file_path = jp.file_path
      ...
      `);
        ...
        () => {
            ...
            try {
                db.exec('ALTER TABLE memove_photos ADD COLUMN passphrase TEXT DEFAULT NULL');
            }
            catch (err) {
                if (!err.message?.includes('duplicate column name'))
                    throw err;
      ...
      `);
        ...
        () => db.exec(`
      CREATE TABLE IF NOT EXISTS memove_photo_cache_meta (
        cache_key  TEXT    PRIMARY KEY,
        content_type TEXT  NOT NULL DEFAULT 'image/jpeg',
        fetched_at INTEGER NOT NULL
      );
      CREATE INDEX IF NOT EXISTS idx_memove_photo_cache_meta_fetched_at ON memove_photo_cache_meta (fetched_at);
    `),
      ...
      `);
        ...
        () => {
            try {
                const hasTable = db
                    .prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name IN ('photo_providers', 'photo_provider_fields')")
                    .all();
            ...
            `).run();
                ...
                if (!err.message?.includes('no such table') && !err.message?.includes('FOREIGN KEY'))
                    throw err;
            }
        },
      ...
      `);
            try {
                ...
                const cols = db.prepare("PRAGMA table_info('day_accommodations')").all();
      ...
      `);
            db.exec('CREATE INDEX IF NOT EXISTS idx_journey_entries_order ' +
      ...
      `);
        ...
        () => {
            ...
            try {
                const del = db.prepare(`DELETE FROM place_regions WHERE place_id IN (
      ...
      `);
        ...
        () => {
            db.prepare("UPDATE addons SET name = 'Costs', description = 'Track and split trip expenses' WHERE id = 'budget' AND name = 'Budget Planner'").run();
        },
    ...
    `),
        ...
        () => {
            const rows = db.prepare('SELECT id, region_code, region_name, country_code FROM visited_regions').all();
            if (rows.length === 0)
                return; // nothing marked → skip the bundle read entirely
            ...
            let features;
            try {
                const file = path_1.default.join(__dirname, '..', '..', 'assets', 'atlas', 'admin1.geojson.gz');
                features = JSON.parse(zlib_1.default.gunzipSync(fs_1.default.readFileSync(file)).toString('utf8')).features || [];
            }
            catch {
                features = []; // bundle missing → degrade to the curated crosswalk below
            }
            const validCodes = new Set();
            const nameToCode = new Map(); // `${A2}|${nameLower}` → code
            ...
            for (const f of features) {
                ...
                if (a2 && name)
                    nameToCode.set(`${a2}|${name.toLowerCase()}`, code);
            }
            ...
            const resolve = (row) => {
                if (validCodes.has(row.region_code))
                ...
                if (merged && (validCodes.size === 0 || validCodes.has(merged)))
                    return merged;
                return null;
            };
            ...
            for (const row of rows) {
                ...
                if (!newCode || newCode === row.region_code)
                    continue;
                ...
                const after = db.prepare('SELECT region_code FROM visited_regions WHERE id = ?').get(row.id);
                if (after && after.region_code === row.region_code)
                    del.run(row.id);
            }
        },
        () => {
            ...
            try {
                db.prepare('INSERT OR IGNORE INTO addons (id, name, description, type, icon, enabled, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)').run('airtrail', 'AirTrail', 'Sync flights from your self-hosted AirTrail instance', 'integration', 'Plane', 0, 14);
            }
            catch (err) {
                console.warn('[migrations] Non-fatal migration step failed:', err);
            }
