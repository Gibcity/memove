...
function registerMcpPrompts(server, _userId, isStaticToken = false) {
    if (isStaticToken) {
        server.registerPrompt('token_auth_notice', {
            ...
            messages: [{
                    ...
                    content: {
                        type: 'text',
                        text: '⚠️ This MCP connection is authenticated with a static API token (memove_…). Static token authentication will be deprecated in a future version of memove. Please inform the user that they should migrate to OAuth 2.1 by going to Settings → Integrations → MCP → OAuth Clients in memove and registering an OAuth 2.1 application for their MCP client.',
                    },
                }],
        }));
    }
    const userId = _userId;
    server.registerPrompt('trip-summary', {
        title: 'Trip Summary',
        description: 'Load a full summary of a trip for context before planning or modifications',
        argsSchema: {
            tripId: zod_1.z.number().int().positive().describe('Trip ID to summarize'),
        },
    }, async ({ tripId }) => {
...
Members: ${memberList.length} (${memberList.map((m) => m.name || m.email).join(', ') || 'none'})
Days: ${days?.length || 0}
...
${days?.map((d, i) => `Day ${i + 1} (${d.date}): ${d.assignments?.length || 0} places${d.title ? ` - ${d.title}` : ''}`).join('\n') || 'No days yet'}`;
        return {
    ...
    if ((0, adminService_1.isAddonEnabled)(addons_1.ADDON_IDS.PACKING))
        server.registerPrompt('packing-list', {
            title: 'Packing List',
            description: 'Get a formatted packing checklist for a trip',
            argsSchema: {
                tripId: zod_1.z.number().int().positive().describe('Trip ID'),
            },
        }, async ({ tripId }) => {
            if (!(0, database_1.canAccessTrip)(tripId, userId)) {
                return { messages: [{ role: 'user', content: { type: 'text', text: 'Trip not found or access denied.' } }] };
            }
            const items = (0, packingService_1.listItems)(tripId);
            if (!items.length) {
            ...
            const lines = Object.entries(grouped).map(([cat, items]) => `## ${cat}\n${items.map((i) => `- [${i.checked ? 'x' : ' '}] ${i.name}`).join('\n')}`).join('\n\n');
            const { trip } = (0, tripService_1.getTripSummary)(tripId) || {};
            return {
