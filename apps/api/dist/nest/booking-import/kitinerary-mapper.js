...
exports.mapReservations = mapReservations;
...
// Type mappers
// ---------------------------------------------------------------------------
function mapFlight(r, source) {
    const f = r.reservationFor;
    if (!f)
        return null;
    const depIata = f.departureAirport?.iataCode?.toUpperCase() ?? null;
    const arrIata = f.arrivalAirport?.iataCode?.toUpperCase() ?? null;
    const depAp = depIata ? (0, airportService_1.findByIata)(depIata) : null;
    const arrAp = arrIata ? (0, airportService_1.findByIata)(arrIata) : null;
    ...
    if (depAp) {
        ...
        if (c)
            endpoints.push({ role: 'from', sequence: 0, name: depLabel, code: depIata, lat: c.lat, lng: c.lng, timezone: null, local_time: depTime, local_date: depDate });
    }
    if (arrAp) {
        ...
        if (c)
            endpoints.push({ role: 'to', sequence: 1, name: arrLabel, code: arrIata, lat: c.lat, lng: c.lng, timezone: null, local_time: arrTime, local_date: arrDate });
    }
    return {
        type: 'flight',
        title,
...
function sameConnection(a, b) {
    const fa = a.reservationFor;
    const fb = b.reservationFor;
    if (!fa || !fb)
        return false;
    const arrIata = fa.arrivalAirport?.iataCode?.toUpperCase();
    const depIata = fb.departureAirport?.iataCode?.toUpperCase();
    if (!arrIata || !depIata || arrIata !== depIata)
        return false; // must connect at the same airport
    ...
    if (arrIso && depIso) {
        ...
        if (gapMs < 0 || gapMs > 24 * 3600 * 1000)
            return false;
    }
    return true;
}
/** Collapse several connecting flight legs (same PNR) into one multi-leg booking. */
function mapFlightGroup(legs, source) {
    const flights = legs.map(l => l.reservationFor);
    if (flights.some(f => !f))
        return mapFlight(legs[0], source); // malformed → fall back to single
    const fs = flights;
    const iataOf = (ap) => ap?.iataCode?.toUpperCase() ?? null;
    const makeEndpoint = (ap, role, time, date) => {
        const iata = iataOf(ap);
        const found = iata ? (0, airportService_1.findByIata)(iata) : null;
        const label = found ? (found.city ? `${found.city} (${found.iata})` : found.name) : (ap?.name ?? iata ?? 'Unknown');
        if (found)
            return { role, sequence: 0, name: label, code: found.iata, lat: found.lat, lng: found.lng, timezone: found.tz, local_time: time, local_date: date };
        const c = coords(ap?.geo);
        if (c)
            return { role, sequence: 0, name: label, code: iata, lat: c.lat, lng: c.lng, timezone: null, local_time: time, local_date: date };
        return null;
    };
    const endpoints = [];
    const metaLegs = [];
...
function mapTrain(r, source) {
    const t = r.reservationFor;
    if (!t)
        return null;
    ...
    if (dc)
        endpoints.push({ role: 'from', sequence: 0, name: depName, code: null, lat: dc.lat, lng: dc.lng, timezone: null, local_time: depTime, local_date: depDate });
    if (ac)
        endpoints.push({ role: 'to', sequence: 1, name: arrName, code: null, lat: ac.lat, lng: ac.lng, timezone: null, local_time: arrTime, local_date: arrDate });
    return {
        type: 'train',
        title,
...
function mapBus(r, source) {
    const b = r.reservationFor;
    if (!b)
        return null;
...
function mapLodging(r, source) {
    const l = r.reservationFor;
    if (!l?.name)
        return null;
    const c = coords(l.geo);
    const venue = { name: l.name, ...(c ?? {}), address: formatAddress(l.address) ?? undefined, website: l.url ?? undefined, phone: l.telephone ?? undefined };
...
function mapFood(r, source) {
    const f = r.reservationFor;
    if (!f?.name)
        return null;
    const c = coords(f.geo);
    const venue = { name: f.name, ...(c ?? {}), address: formatAddress(f.address) ?? undefined, website: f.url ?? undefined, phone: f.telephone ?? undefined };
...
function mapRentalCar(r, source) {
    const car = r.reservationFor;
    ...
    const venue = pickup?.name ? { name: pickup.name, ...(pc ?? {}), address: formatAddress(pickup.address) ?? undefined } : undefined;
...
function mapEvent(r, source) {
    const e = r.reservationFor;
    if (!e?.name)
        return null;
    const loc = e.location;
    const c = coords(loc?.geo);
    const venue = loc?.name ? { name: loc.name, ...(c ?? {}), address: formatAddress(loc.address) ?? undefined } : undefined;
...
// Public
// ---------------------------------------------------------------------------
function mapReservations(kiItems, fileName) {
    const items = [];
    const warnings = [];
