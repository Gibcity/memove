...
const CONCIERGE_SYSTEM_PROMPT = 'You are a practical relocation concierge for someone moving within the US. ' +
...
let ConciergeService = class ConciergeService {
    ...
    async handleQuery(userId, query) {
        ...
        if (isGeneral) {
            try {
                const answer = await (0, client_1.complete)([
    ...
    classifyQuery(query) {
        ...
        if (/scam|fraud|legit|fake|trust|reliable/.test(q))
            return 'safety';
        if (/pet|dog|cat|animal/.test(q))
            return 'pets';
        if (/insur|cover|claim|deductible/.test(q))
            return 'insurance';
        if (/utility|electric|water|gas|internet|wifi/.test(q))
            return 'utilities';
        if (/storage|unit|pod/.test(q))
            return 'storage';
        if (/clean|paint|repair|fix|damage/.test(q))
            return 'maintenance';
        if (/bank|account|transfer|direct deposit/.test(q))
            return 'banking';
        if (/mail|address|forward|usps/.test(q))
            return 'mail';
        if (/lonely|friend|community|meet|social/.test(q))
            return 'social';
        if (/stress|anxious|worried|overwhelm/.test(q))
            return 'emotional';
        return 'general';
    }
    buildResponse(query, category) {
