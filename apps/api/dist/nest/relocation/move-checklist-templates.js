...
exports.MOVE_CHECKLIST_TEMPLATES = exports.stateTexas = exports.familyWithKids = exports.firstTimerBaseline = void 0;
exports.selectChecklistTasks = selectChecklistTasks;
const PRIORITY_NORMAL = 0;
const PRIORITY_HIGH = 1;
const PRIORITY_URGENT = 2;
// ── 1. First-timer baseline ────────────────────────────────────────────────
exports.firstTimerBaseline = {
    id: 'first-timer-baseline',
    name: 'First-Time Mover Baseline',
    description: 'Core tasks every first-time mover needs, from neighborhood research through post-move settlement.',
    tasks: [
        // Housing
...
// ── 2. Family with kids ───────────────────────────────────────────────────
exports.familyWithKids = {
    id: 'family-with-kids',
    name: 'Family with Kids — Extra Tasks',
    description: 'Additional tasks when relocating with children. Apply alongside the baseline.',
    tasks: [
...
exports.stateTexas = {
    ...
    description: 'Texas has no state income tax but specific vehicle and property tax rules. Apply on top of the baseline when moving to TX.',
    tasks: [
...
// ── Aggregate export ──────────────────────────────────────────────────────
exports.MOVE_CHECKLIST_TEMPLATES = [
    exports.firstTimerBaseline,
    exports.familyWithKids,
    exports.stateTexas,
];
/** Flatten templates and filter tasks by appliesTo rules. Pure data routing. */
function selectChecklistTasks(ctx) {
    const out = [];
    for (const tpl of exports.MOVE_CHECKLIST_TEMPLATES) {
        for (const task of tpl.tasks) {
            ...
            if (a.firstTimeOnly && !ctx.firstTimeMover)
                continue;
            if (a.hasPets && !ctx.hasPets)
                continue;
            if (a.destinationStates && (!ctx.destinationState || !a.destinationStates.includes(ctx.destinationState)))
                continue;
            if (a.demographics && (!ctx.demographics || !a.demographics.some(d => ctx.demographics.includes(d))))
                continue;
            out.push(task);
        }
    }
