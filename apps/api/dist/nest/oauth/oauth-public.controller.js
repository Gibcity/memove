...
exports.OauthPublicController = void 0;
 ...
 */
let OauthPublicController = class OauthPublicController {
    oauth;
    rl;
    ...
    token(req, res) {
        ...
        if (grant_type === 'authorization_code') {
            ...
            const invalidGrant = (reason, userId) => {
                (0, auditLog_1.writeAudit)({ userId, action: 'oauth.token.grant_failed', details: { client_id, reason }, ip });
                res.status(400).json({ error: 'invalid_grant', error_description: 'Authorization grant is invalid.' });
            };
            if (!pending)
                return invalidGrant('code_invalid_or_expired', null);
            if (pending.clientId !== client_id)
                return invalidGrant('client_id_mismatch', pending.userId);
            if (pending.redirectUri !== redirect_uri)
                return invalidGrant('redirect_uri_mismatch', pending.userId);
            ...
            if (!this.oauth.authenticateClient(client_id, client_secret)) {
                (0, auditLog_1.logWarn)(`[OAuth] Invalid client credentials for client_id=${client_id} ip=${ip ?? '-'}`);
                (0, auditLog_1.writeAudit)({ userId: pending.userId, action: 'oauth.token.client_auth_failed', details: { client_id }, ip });
            ...
            (0, auditLog_1.writeAudit)({ userId: pending.userId, action: 'oauth.token.issue', details: { client_id, scopes: pending.scopes, audience: pending.resource ?? null }, ip });
            res.json(tokens);
            return;
        }
        if (grant_type === 'refresh_token') {
            ...
            if (result.error) {
                if (result.error === 'invalid_client')
                    (0, auditLog_1.logWarn)(`[OAuth] Invalid client credentials on refresh for client_id=${client_id} ip=${ip ?? '-'}`);
        ...
        if (grant_type === 'client_credentials') {
            ...
            if (!client) {
                (0, auditLog_1.logWarn)(`[OAuth] Invalid client credentials for client_id=${client_id} ip=${ip ?? '-'}`);
                (0, auditLog_1.writeAudit)({ userId: null, action: 'oauth.token.client_auth_failed', details: { client_id }, ip });
                res.status(401).json({ error: 'invalid_client', error_description: 'Invalid client credentials' });
                return;
            }
            if (client.is_public || !client.allows_client_credentials || client.user_id == null) {
                (0, auditLog_1.writeAudit)({ userId: client.user_id ?? null, action: 'oauth.token.grant_failed', details: { client_id, reason: 'unauthorized_client' }, ip });
            ...
            (0, auditLog_1.writeAudit)({ userId: client.user_id, action: 'oauth.token.issue', details: { client_id, scopes: grantedScopes, audience, grant: 'client_credentials' }, ip });
            res.json(tokens);
            return;
        }
    ...
    userinfo(auth, res) {
        ...
        if (!auth || !auth.toLowerCase().startsWith('bearer ')) {
            res.set('WWW-Authenticate', 'Bearer realm="memove MCP"');
            res.status(401).json({ error: 'invalid_token' });
            return;
        }
        const info = this.oauth.getUserByAccessToken(auth.slice(7));
        if (!info) {
            res.set('WWW-Authenticate', 'Bearer realm="memove MCP", error="invalid_token"');
            res.status(401).json({ error: 'invalid_token' });
            return;
        }
    ...
    revoke(req, res) {
        ...
        if (!this.oauth.authenticateClient(client_id, client_secret)) {
            (0, auditLog_1.logWarn)(`[OAuth] Invalid client credentials on revoke for client_id=${client_id} ip=${ip ?? '-'}`);
            (0, auditLog_1.writeAudit)({ userId: null, action: 'oauth.token.client_auth_failed', details: { client_id, endpoint: 'revoke' }, ip });
            res.status(401).json({ error: 'invalid_client', error_description: 'Invalid client credentials' });
            return;
        }
...
exports.OauthPublicController = OauthPublicController;
__decorate([
    (0, common_1.Post)('token'),
    (0, common_1.HttpCode)(200) // token success uses res.json without an explicit status; Express defaults to 200 (Nest POST would default to 201).
    ,
...
], OauthPublicController.prototype, "token", null);
__decorate([
    ...
    __metadata("design:returntype", void 0)
], OauthPublicController.prototype, "userinfo", null);
__decorate([
    ...
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", void 0)
], OauthPublicController.prototype, "revoke", null);
exports.OauthPublicController = OauthPublicController = __decorate([
