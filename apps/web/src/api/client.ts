...
/**
 * Validate a response payload against its @memove/shared Zod schema — but only in
 ...
 * the FE adopts per domain as each backend module lands on @memove/shared.
 */
...
export function parseInDev<S extends z.ZodTypeAny>(schema: S, data: unknown, label: string): z.infer<S> {
  if (API_DEV) {
    ...
    if (!result.success) {
      console.warn(`[api] ${label}: response did not match the @memove/shared schema`, result.error.issues)
    }
  }
...
apiClient.interceptors.response.use(
    ...
    async (error) => {
      ...
      // HTML body (a JS redirect page) instead of a 302. memove's own 401s are
