...
import type { TripCreateRequest } from '@memove/shared'
import {
  type DashboardTrip,
  type TravelStats,
...
export function useDashboard() {
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'list'>(() => (localStorage.getItem('memove_dashboard_view') as 'grid' | 'list') || 'grid')
  ...
  const toggleViewMode = () => {
    setViewMode(prev => {
      ...
      localStorage.setItem('memove_dashboard_view', next)
      return next
    })
  }
