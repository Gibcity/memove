...
function endpointIcon(type: TransportType, label: string | null): L.DivIcon {
  ...
  return L.divIcon({
    className: 'memove-endpoint-marker',
    html: `<div style="
...
function StatsLabel({ item }: { item: TransportItem }) {
  ...
  const apply = () => {
    ...
    if (!innerRef.current && el) innerRef.current = el.querySelector('.memove-stats-inner') as HTMLElement | null
    if (innerRef.current) innerRef.current.style.transform = `rotate(${pose.angle}deg)`
  }

  useEffect(() => {
    const icon = L.divIcon({
      className: 'memove-endpoint-stats',
      html,
