  ...
  it('FE-COMP-NOTES-039: NoteFormModal website field accepts URL input', async () => {
    ...
    server.use(
      http.post('/api/trips/1/collab/notes', async ({ request }) => {
        ...
        return HttpResponse.json({
          note: { id: 99, trip_id: 1, user_id: 1, author_username: 'testuser', author_avatar: null, title: 'URL Note', content: '', category: null, color: '#3b82f6', website: 'https://memove.app', files: [], attachments: [], created_at: new Date().toISOString(), updated_at: new Date().toISOString() },
        });
      })
    );
    render(<CollabNotes {...defaultProps} />);
    ...
    await user.type(websiteInput, 'https://memove.app');
    await user.click(screen.getByRole('button', { name: /^Create$/i }));
    await waitFor(() => expect(postBody.website).toBe('https://memove.app'));
  });
      ...
      http.get('/api/trips/1/collab/notes', () =>
        HttpResponse.json({
          notes: [{
            ...
            website: 'https://memove-app.example.com', files: [], attachments: [],
            created_at: '2025-06-01T10:00:00.000Z', updated_at: '2025-06-01T10:00:00.000Z',
          }],
        })
      ),
      http.get('/api/trips/1/collab/link-preview', () =>
        HttpResponse.json({ title: 'memove App', image: 'https://memove-app.example.com/og.jpg' })
      ),
    );
    render(<CollabNotes {...defaultProps} />);
